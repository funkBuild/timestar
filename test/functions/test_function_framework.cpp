...
TEST_F(FunctionFrameworkTest, ExceptionHierarchy) {
    ...
    // Test inheritance
    {
        [[maybe_unused]] FunctionException& r = paramEx;
        (void)r;
    }
    {
        [[maybe_unused]] std::runtime_error& r = baseEx;
        (void)r;
    }
}
...
public:
    ...
    std::unique_ptr<IFunction> clone() const override { return std::make_unique<MockFunction>(); }
