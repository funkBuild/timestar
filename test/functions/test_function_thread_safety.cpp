...
protected:
    ...
    void TearDown() override { FunctionRegistry::getInstance().clear(); }
...
TEST_F(FunctionThreadSafetyTest, ConcurrentFunctionExecution) {
    ...
    EXPECT_EQ(totalExecutions, numThreads * executionsPerThread) << "Not all executions completed successfully";
    EXPECT_EQ(functionCallCounter.load(), numThreads * executionsPerThread) << "Function call counter mismatch";
}
...
TEST_F(FunctionThreadSafetyTest, ConcurrentFunctionCloning) {
    ...
    EXPECT_EQ(cloneCounter.load(), numThreads * clonesPerThread * 5) << "Not all clones created successfully";
}
...
TEST_F(FunctionThreadSafetyTest, AtomicOperationsAndMemoryCoherency) {
    ...
    EXPECT_EQ(atomicCounter.load(), numThreads * operationsPerThread) << "Atomic counter mismatch";

    // Check memory coherency
    long long expectedSum = atomicSum.load();
    EXPECT_EQ(volatileSum, expectedSum) << "Memory coherency issue detected between atomic and volatile variables";
}
...
TEST_F(FunctionThreadSafetyTest, PerformanceTrackingRaceConditions) {
    ...
    struct PerformanceTracker {
        ...
        void recordExecution(long long executionTime) {
            ...
            while (executionTime < current_min && !minExecutionTime.compare_exchange_weak(current_min, executionTime)) {
                // Retry if compare_exchange failed
            }

            long long current_max = maxExecutionTime.load();
            while (executionTime > current_max && !maxExecutionTime.compare_exchange_weak(current_max, executionTime)) {
                // Retry if compare_exchange failed
            }
        }
    ...
    auto worker = [this, executionsPerThread, &tracker](int threadId) {
        ...
        try {
            for (int i = 0; i < executionsPerThread; ++i) {
                ...
                if (function) {
                    FunctionContext context =
                        createContextForFunction(functionName, static_cast<double>(threadId + 1.0));

                    // Choose input data based on thread ID
                    const std::vector<uint64_t>* timestamps =
                        largeTimestamps.empty() ? &testTimestamps1 : &largeTimestamps;
    ...
    EXPECT_GT(tracker.totalExecutionTime.load(), 0) << "Total execution time should be positive";
    ...
    EXPECT_GT(tracker.getAverageExecutionTime(), 0) << "Average execution time should be positive";
}
...
TEST_F(FunctionThreadSafetyTest, ProductionLikeScenario) {
    ...
    auto httpWorker = [this, &stopFlag, &stats](int workerId) {
        ...
        while (!stopFlag.load()) {
            try {
                ...
                const std::vector<uint64_t>* timestamps =
                    (useLargeData && !largeTimestamps.empty()) ? &largeTimestamps : &testTimestamps1;
                const std::vector<double>* values =
                    (useLargeData && !largeValues.empty()) ? &largeValues : &testValues1;
                DoubleSeriesView inputView(timestamps, values);
    ...
    double successRate =
        static_cast<double>(stats.successfulRequests.load()) / static_cast<double>(stats.totalRequests.load());
