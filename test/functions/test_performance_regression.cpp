...
protected:
    ...
    void checkRegression(const std::string& test_name, const PerformanceResult& result) {
        ...
        if (time_regression > 0.50) {
            GTEST_FAIL() << "Performance regression detected in " << test_name << ": " << (time_regression * 100)
        ...
        if (baseline_memory > 1000) {  // Only check if baseline had meaningful memory usage
            ...
            if (memory_regression > 2.0) {
                GTEST_FAIL() << "Memory regression detected in " << test_name << ": " << (memory_regression * 100)
                             << "% more memory than baseline"
                             << " (baseline: " << baseline_memory << " bytes, current: " << result.memory_usage_bytes
                             << " bytes)";
            }
        }

        std::cout << "Performance check passed for " << test_name << " (time: " << result.execution_time_ms
                  << "ms, throughput: " << result.throughput_points_per_sec << " pts/sec)\n";
    }
...
TEST_F(PerformanceRegressionTest, MemoryScaling_DataProcessing) {
    ...
    std::cout << "Memory scaling: Small=" << small_result.memory_usage_bytes
              << ", Medium=" << medium_result.memory_usage_bytes << ", Large=" << large_result.memory_usage_bytes
              << std::endl;

    // Memory should scale reasonably with data size
    // These are soft checks - we mainly want to detect massive memory leaks
    EXPECT_LT(medium_result.memory_usage_bytes, 50 * 1024 * 1024) << "Medium dataset should use <50MB memory";
    EXPECT_LT(large_result.memory_usage_bytes, 200 * 1024 * 1024) << "Large dataset should use <200MB memory";
}
...
TEST_F(PerformanceRegressionTest, ThroughputScaling_DataProcessing) {
    ...
    EXPECT_GT(medium_result.throughput_points_per_sec, 1000.0) << "Medium dataset throughput should be reasonable";
    EXPECT_GT(large_result.throughput_points_per_sec, 1000.0) << "Large dataset throughput should be reasonable";
}
...
TEST_F(PerformanceRegressionTest, ConcurrentExecution_Performance) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&, t]() {
            ...
            for (int i = 0; i < executions_per_thread; ++i) {
                measureDataProcessingPerformance("SMA_Concurrent_" + std::to_string(t), medium_dataset, "sma");
            }
    ...
    std::cout << "Concurrent execution completed - avg time: " << avg_thread_time << "ms, max time: " << max_thread_time
              << "ms\n";
...
TEST_F(PerformanceRegressionTest, MemoryLeak_StressTest) {
    ...
    for (int i = 0; i < 100; ++i) {
        measureDataProcessingPerformance("SMA_Stress_" + std::to_string(i), small_dataset, "sma");
    ...
    std::cout << "Memory leak stress test completed - initial: " << initial_memory << " bytes, final: " << final_memory
              << " bytes\n";
}
