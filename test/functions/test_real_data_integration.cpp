...
TEST_F(RealDataIntegrationTest, DataIntegrityAfterTSMStorage) {
    ...
    auto retrievedData = queryRealData("integrity_test", "value", tags, 
                                     baseTime, baseTime + (200 * 60 * 1000000000ULL));
    
    // Verify data integrity
    ASSERT_EQ(retrievedData.size(), originalTimestamps.size()) 
        << "Data size changed after TSM storage";
    
    EXPECT_EQ(retrievedData.timestamps, originalTimestamps) 
        << "Timestamps changed after TSM storage";
...
TEST_F(RealDataIntegrationTest, LargeDatasetFunctionOperations) {
    ...
    auto largeResult = queryRealData("performance", "metric", tags, 
                                   baseTime, baseTime + (8 * 24 * 3600 * 1000000000ULL));
    ...
    std::cout << "Large dataset (" << dataPoints << " points) processing time: " 
