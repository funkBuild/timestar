...
protected:
    ...
    void setupEdgeCaseData() {
        ...
        precisionValues = {1.0000000000000002,  // Small precision difference
                           1.0000000000000004, 1.0000000000000006, 1.0000000000000008, 1.000000000000001};
    ...
    // Test data sets
    std::vector<uint64_t> emptyTimestamps, singleTimestamp, extremeTimestamps, precisionTimestamps, boundaryTimestamps,
        largeTimestamps;
    std::vector<double> emptyValues, singleValue, extremeValues, precisionValues, boundaryValues, largeValues;
};
...
TEST_F(EdgeCasesTest, ArithmeticFunctionsWithExtremeValues) {
    ...
    EXPECT_TRUE(std::isfinite(addResult.values[0]) ||
                (std::isinf(addResult.values[0]) && addResult.values[0] > 0));  // max + 1
    EXPECT_TRUE(addResult.values[1] > 0);                                       // min + 1 should be positive
    EXPECT_TRUE(std::isfinite(addResult.values[2]) ||
                (std::isinf(addResult.values[2]) && addResult.values[2] < 0));  // -max + 1
...
TEST_F(EdgeCasesTest, TimestampOverflowBoundaries) {
    std::vector<uint64_t> overflowTimestamps = {std::numeric_limits<uint64_t>::max() - 1,
