    ...
    void startMonitoring() {
        ...
        monitorThread_ = std::thread([this]() {
            while (monitoring_) {
                ...
                while (currentMem > currentPeak && !peakMemoryUsage_.compare_exchange_weak(currentPeak, currentMem)) {
...
public:
    ...
    ~MemoryLeakDetector() { monitor_.stopMonitoring(); }
...
public:
    ...
    std::chrono::microseconds getElapsedTime() const {
        return std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - startTime_);
    }
};
...
public:
    ...
    const FunctionMetadata& getMetadata() const override {
        static FunctionMetadata metadata{"memory_intensive",
    ...
    std::unique_ptr<IFunction> clone() const override { return std::make_unique<MemoryIntensiveFunction>(); }
};
...
public:
    ...
    std::unique_ptr<IFunction> clone() const override { return std::make_unique<CPUIntensiveFunction>(); }
};
...
protected:
    void SetUp() override {
        ...
        FunctionRegistry::getInstance().registerFunction<CPUIntensiveFunction>(CPUIntensiveFunction().getMetadata());
...
TEST_F(FunctionResourceManagementTest, CPUUsageLimits) {
    ...
    std::future<FunctionResult<double>> futureResult =
        std::async(std::launch::async, [&]() { return unaryFunction->execute(testSeriesView_, context).get(); });
