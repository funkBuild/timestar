...
public:
    void recordInitialMemory() { initialMemoryUsage_ = getCurrentRSS(); }

    size_t getCurrentMemoryUsage() const { return getCurrentRSS(); }
...
public:
    ...
    std::unique_ptr<IFunction> clone() const override { return std::make_unique<TestMemoryIntensiveFunction>(); }
};
...
public:
    ...
    std::unique_ptr<IFunction> clone() const override { return std::make_unique<TestCPUIntensiveFunction>(); }
};
...
TEST(ResourceManagementBasicTest, CPUUsageAndPerformance) {
    ...
    EXPECT_LT(executionMs, 10000) << "CPU-intensive function should complete within 10 seconds: " << executionMs
                                  << "ms";
...
TEST(ResourceManagementBasicTest, ConcurrentFunctionExecution) {
    ...
    EXPECT_GE(successfulExecutions.load(), 3)
        << "Most concurrent executions should succeed: " << successfulExecutions.load() << " successful, "
