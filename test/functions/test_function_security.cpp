...
protected:
    ...
    std::string createMockHttpRequest(const std::string& query, const std::string& functionName = "",
...
TEST_F(FunctionSecurityTest, FunctionNameValidation) {
    ...
    for (const auto& functionName : maliciousFunctionNames) {
        ...
        EXPECT_NO_THROW({
            ...
            bool isDangerous =
                (testName.find("../") != std::string::npos || testName.find("..\\") != std::string::npos ||
                 testName.find("system") != std::string::npos || testName.find("exec") != std::string::npos ||
                 testName.find("shell") != std::string::npos || testName == "admin" || testName == "root" ||
                 testName.find("drop") != std::string::npos || testName.find("delete") != std::string::npos);
...
TEST_F(FunctionSecurityTest, JSONParsingSecurityBoundaryTests) {
    ...
    std::vector<std::string> maliciousJsonInputs = {
        ...
        // Null bytes in JSON
        std::string("{\"functionName\":\"sma") + std::string(1, '\0') +
            std::string("\",\"parameters\":{\"window\":5}}"),
            ...
            if (queryPos != std::string::npos) {
                ...
                if (start != std::string::npos) {
                    ...
                    if (end != std::string::npos) {
                        ...
                        bool hasDangerousPatterns = (lowerQuery.find("system") != std::string::npos ||
...
TEST_F(FunctionSecurityTest, AuthenticationBypassAttempts) {
    ...
    for (const auto& payload : authBypassPayloads) {
        ...
        EXPECT_NO_THROW({
            ...
            if (content.find("admin") != std::string::npos || content.find("token") != std::string::npos ||
                content.find("auth") != std::string::npos || content.find("privilege") != std::string::npos ||
                content.find("sudo") != std::string::npos || content.find("root") != std::string::npos) {
...
TEST_F(FunctionSecurityTest, ReDoSProtectionXSSPattern) {
    ...
    while (static_cast<int>(attack.size()) < maxJsonLen)
        attack += "ona";
    ...
    EXPECT_LT(ms, 200) << "containsDangerousPatterns() took " << ms << "ms on " << maxJsonLen
    ...
    EXPECT_FALSE(FunctionSecurity::containsDangerousPatterns("on" + std::string(31, 'a') + "=x"));
