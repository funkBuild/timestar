...
| `pattern_delete_failover_gate.sh` | an ambiguous coordinator crash only after another Group-0 voter applies a complete 6,000-target frozen plan, bounded byte-identical retry of transport failures/503s under a new leader without capturing a newly matching series, changed-body conflict, and killed-node restart recovery |
...
| `multi_host_candidate_preflight.py` | HTTPS-only read-only preflight for an already deployed production candidate; proves distinct endpoint and inter-node peer hosts/failure domains and persistent node identities, one exact clean server revision/cluster UUID/map epoch, the candidate's reactor count, observed process-wide Seastar allocation and aggregate proposal budget, complete all-voter RF=3 hosting/leadership, healthy Group 0, and zero apply/durability faults before and after infrastructure fault arms |
| `multi_host_evidence_bundle.py` | exact-v1 before/after binder for one distinct-host arm; ordinary recovery rejects a changed candidate, deployment, cluster, persistent node/peer/failure-domain/map topology, while backup/restore instead requires a fresh cluster UUID, persistent node UUIDs, resolved endpoints, and peer addresses disjoint from the source; every arm rejects missing per-node logs, reused evidence files, or non-monotonic timestamps and records bounded streaming SHA-256 identities for both preflights and every required transcript |
