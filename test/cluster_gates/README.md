...
| `multi_host_candidate_preflight.py` | HTTPS-only read-only preflight for an already deployed production candidate; proves distinct endpoint and inter-node peer hosts/failure domains, one exact clean server revision/cluster UUID/map epoch, the candidate's reactor count, observed process-wide Seastar allocation and aggregate proposal budget, complete all-voter RF=3 hosting/leadership, healthy Group 0, and zero apply/durability faults before and after infrastructure fault arms |
