...
TEST_F(BackfillTimeoutConfigTest, DefaultQueryTimeoutIsUsableAsBackfillTimeout) {
    ...
    EXPECT_GT(t.count(), 0)
        << "Backfill timeout must be positive";
    EXPECT_LE(t.count(), 300)
        << "Backfill timeout should not exceed 5 minutes";
}
...
TEST_F(BackfillTimeoutConfigTest, DefaultQueryTimeoutIs30Seconds) {
    ...
    EXPECT_EQ(HttpQueryHandler::defaultQueryTimeout().count(), 30)
        << "Default backfill timeout should be 30 seconds";
}
...
TEST_F(BackfillTimeoutConfigTest, SeastarWithTimeoutHeaderAvailable) {
    ...
    static_assert(
        std::is_convertible_v<std::chrono::seconds, ClockDur>,
...
TEST_F(BackfillTimeoutConfigTest, TimeoutIsConsistentAcrossMultipleBackfillQueries) {
    ...
    EXPECT_EQ(t1.count(), t2.count())
        << "Timeout value must be stable across per-query iterations";
}
...
TEST_F(BackfillTimeoutConfigTest, TimedOutErrorInheritsFromStdException) {
    ...
    static_assert(
        std::is_base_of_v<std::exception, seastar::timed_out_error>,
...
TEST_F(BackfillTimeoutConfigTest, WithTimeoutUsedInStreamHandlerSource) {
    ...
    ASSERT_TRUE(src.is_open())
        << "Could not open " HTTP_STREAM_HANDLER_SOURCE_PATH;
    ...
    EXPECT_TRUE(found)
        << "seastar::with_timeout not found in http_stream_handler.cpp; "
