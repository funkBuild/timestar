...
SEASTAR_TEST_F(LevelDBIndexPatternTest, WildcardStarMatch) {
    ...
    auto id1 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-01"}}, "usage");
    auto id2 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-02"}}, "usage");
    auto id3 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-10"}}, "usage");
    auto id4 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "client-01"}}, "usage");
...
SEASTAR_TEST_F(LevelDBIndexPatternTest, ExactMatchBackwardCompat) {
    ...
    auto id1 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-01"}}, "usage");
    co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-02"}}, "usage");
...
SEASTAR_TEST_F(LevelDBIndexPatternTest, FindSeriesDispatchesPattern) {
    ...
    auto id1 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-01"}, {"dc", "dc1"}}, "usage");
    auto id2 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-02"}, {"dc", "dc1"}}, "usage");
    auto id3 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "server-03"}, {"dc", "dc2"}}, "usage");
    auto id4 = co_await index.getOrCreateSeriesId(measurement,
        {{"host", "client-01"}, {"dc", "dc1"}}, "usage");

    // Pattern filter: host:server-* AND dc:dc1
    // Should match server-01 and server-02 (server-03 is dc2, client-01 doesn't match host)
    std::map<std::string, std::string> tagFilters = {
        {"host", "server-*"},
        {"dc", "dc1"}
    };
