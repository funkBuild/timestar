...
// ---------------------------------------------------------------------------
static bool hasBareCall(const std::string& methodBody, const std::string& pattern) {
    size_t searchStart = 0;
    while (true) {
        auto pos = methodBody.find(pattern, searchStart);
        if (pos == std::string::npos)
            return false;  // pattern not found at all
    ...
    void SetUp() override {
        {
            ...
            ASSERT_TRUE(file.is_open()) << "Could not open wal_file_manager.cpp at: "
                                        << WAL_FILE_MANAGER_CPP_SOURCE_PATH;
            walSource.assign(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>());
        ...
        }
        {
            ...
            engineSource.assign(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>());
            ASSERT_FALSE(engineSource.empty());
        }
        {
            std::ifstream file(LEVELDB_INDEX_CPP_SOURCE_PATH);
            ASSERT_TRUE(file.is_open()) << "Could not open leveldb_index.cpp at: " << LEVELDB_INDEX_CPP_SOURCE_PATH;
            indexSource.assign(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>());
...
TEST_F(WALInitBlockingTest, WALInitNoBlockingFsExists) {
    ...
    ASSERT_FALSE(initBody.empty()) << "Could not extract WALFileManager::init() method body";

    bool hasBare = hasBareCall(initBody, "fs::exists");

    EXPECT_FALSE(hasBare) << "Found bare 'fs::exists' call in WALFileManager::init() outside of "
...
TEST_F(WALInitBlockingTest, WALInitNoBlockingDirectoryIterator) {
    ...
    EXPECT_FALSE(hasBare) << "Found bare 'fs::directory_iterator' call in WALFileManager::init() "
...
TEST_F(WALInitBlockingTest, WALInitUsesAsyncForDirectoryScan) {
    ...
    EXPECT_TRUE(hasAsync) << "Expected 'seastar::async' in WALFileManager::init() to wrap "
...
TEST_F(WALInitBlockingTest, EngineNoBlockingCreateDirectories) {
    std::string createDirBody = extractMethodBody(engineSource, "Engine::createDirectoryStructure()");
    ASSERT_FALSE(createDirBody.empty()) << "Could not extract Engine::createDirectoryStructure() method body";

    bool hasBare = hasBareCall(createDirBody, "fs::create_directories");

    EXPECT_FALSE(hasBare) << "Found bare 'fs::create_directories' call in "
...
TEST_F(WALInitBlockingTest, LevelDBIndexNoBlockingCreateDirectories) {
    std::string openBody = extractMethodBody(indexSource, "LevelDBIndex::open()");
    ASSERT_FALSE(openBody.empty()) << "Could not extract LevelDBIndex::open() method body";

    bool hasBare = hasBareCall(openBody, "std::filesystem::create_directories");

    EXPECT_FALSE(hasBare) << "Found bare 'std::filesystem::create_directories' call in "
