...
protected:
    ...
    void SetUp() override {
        ...
        ASSERT_TRUE(file.is_open())
            << "Could not open wal_file_manager.hpp at: " << WAL_FILE_MANAGER_SOURCE_PATH;
        sourceCode.assign(
...
TEST_F(WALFileManagerCloseTest, CloseIteratesAllStores) {
    ...
    bool hasForLoop = (closeBody.find("for (") != std::string::npos ||
                       closeBody.find("for(") != std::string::npos);
    bool iteratesMemoryStores = (closeBody.find("memoryStores") != std::string::npos &&
                                 hasForLoop);

    EXPECT_TRUE(iteratesMemoryStores)
        << "The close() method must use a loop (for/while) to iterate over "
...
TEST_F(WALFileManagerCloseTest, CloseHandlesErrorsPerStore) {
    ...
    if (forPos == std::string::npos) forPos = closeBody.find("for(");
    ...
    while (i < closeBody.size() && depth > 0) {
        if (closeBody[i] == '{') depth++;
        else if (closeBody[i] == '}') depth--;
        i++;
    }
    ...
    EXPECT_TRUE(hasTry && hasCatch)
        << "The for loop in close() must contain a try/catch block inside "
...
TEST_F(WALFileManagerCloseTest, CloseLogsEachStore) {
    ...
    bool hasInfoLog = (loopBody.find("wal_log.info") != std::string::npos ||
                       loopBody.find("log.info") != std::string::npos);

    EXPECT_TRUE(hasInfoLog)
        << "The for loop in close() must log each memory store closure "
...
TEST_F(WALFileManagerCloseTest, CloseNotJustFirstStore) {
    ...
    EXPECT_FALSE(hasDirectIndexAccess)
        << "Found 'memoryStores[0]' in close() method. The close() method "
