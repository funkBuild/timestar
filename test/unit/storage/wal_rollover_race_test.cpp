...
static size_t findPattern(const std::string &body, const std::string &pattern,
                          size_t startPos = 0) {
...
class WALRolloverRaceTest : public ::testing::Test {
  protected:
    ...
    void SetUp() override {
        ...
        rolloverBody = extractMethodBody(
            source, "WALFileManager::rolloverMemoryStore()");
        ASSERT_FALSE(rolloverBody.empty())
            << "Could not extract WALFileManager::rolloverMemoryStore() body";
    }
};
...
TEST_F(WALRolloverRaceTest, NewStoreCreatedBeforeOldStoreClosed) {
    size_t makeSharedPos =
        findPattern(rolloverBody, "make_shared<MemoryStore>");
    size_t closePos = findPattern(rolloverBody, "previousStore->close()");

    ASSERT_NE(makeSharedPos, std::string::npos)
        << "Could not find 'make_shared<MemoryStore>' in rolloverMemoryStore()";
    ASSERT_NE(closePos, std::string::npos)
        << "Could not find 'previousStore->close()' in rolloverMemoryStore()";

    EXPECT_LT(makeSharedPos, closePos)
        << "make_shared<MemoryStore> must appear BEFORE previousStore->close() "
...
TEST_F(WALRolloverRaceTest, InitWALBeforeOldStoreClosed) {
    ...
    ASSERT_NE(initWalPos, std::string::npos)
        << "Could not find 'store->initWAL()' in rolloverMemoryStore()";
    ASSERT_NE(closePos, std::string::npos)
        << "Could not find 'previousStore->close()' in rolloverMemoryStore()";

    EXPECT_LT(initWalPos, closePos)
        << "store->initWAL() must appear BEFORE previousStore->close(). "
...
TEST_F(WALRolloverRaceTest, StoreInstalledBeforeOldStoreClosed) {
    size_t insertPos =
        findPattern(rolloverBody, "memoryStores.insert(memoryStores.begin()");
    size_t closePos = findPattern(rolloverBody, "previousStore->close()");

    ASSERT_NE(insertPos, std::string::npos)
        << "Could not find 'memoryStores.insert(memoryStores.begin()' in "
        << "rolloverMemoryStore()";
    ASSERT_NE(closePos, std::string::npos)
        << "Could not find 'previousStore->close()' in rolloverMemoryStore()";

    EXPECT_LT(insertPos, closePos)
        << "memoryStores.insert() must appear BEFORE previousStore->close(). "
...
TEST_F(WALRolloverRaceTest, FullOrderingIsCorrect) {
    ...
    ASSERT_NE(makeSharedPos, std::string::npos)
        << "Could not find 'make_shared<MemoryStore>'";
    ASSERT_NE(initWalPos, std::string::npos)
        << "Could not find 'store->initWAL()'";
    ASSERT_NE(insertPos, std::string::npos)
        << "Could not find 'memoryStores.insert(memoryStores.begin()'";
    ASSERT_NE(closePos, std::string::npos)
        << "Could not find 'previousStore->close()'";

    EXPECT_LT(makeSharedPos, initWalPos)
        << "make_shared<MemoryStore> must come before store->initWAL()";
    EXPECT_LT(initWalPos, insertPos)
        << "store->initWAL() must come before memoryStores.insert()";
    EXPECT_LT(insertPos, closePos)
        << "memoryStores.insert() must come before previousStore->close(). "
