...
protected:
    ...
    seastar::shared_ptr<TSM> createTestTSMFile(
        ...
        char filename[256];
        snprintf(filename, sizeof(filename), "shard_0/%02lu_%010lu.tsm", tier, seqNum);
        
        TSMWriter writer(filename);
        
        for (int s = 0; s < numSeries; s++) {
            std::string seriesId = seriesPrefix + std::to_string(s);
...
TEST_F(TSMCompactorTest, BasicCompaction) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        // Create 4 tier-0 files
        std::vector<seastar::shared_ptr<TSM>> files;
        for (int i = 0; i < 4; i++) {
            auto tsm = createTestTSMFile(0, i, "sensor.", 5, 100, 1000000 + i * 100000);
            co_await tsm->open();
            co_await tsm->readIndex();
            files.push_back(tsm);
            fileManager->sequencedTsmFiles[i] = tsm;
        }
        
        // Compact the files
        auto compactedFile = co_await compactor->compact(files);
        
        EXPECT_FALSE(compactedFile.empty());
        EXPECT_TRUE(fs::exists(compactedFile));
        
        // Verify compacted file has expected structure
        EXPECT_TRUE(verifyCompactedFile(compactedFile, 5, 400));
...
TEST_F(TSMCompactorTest, DeduplicationDuringCompaction) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        for (int i = 0; i < 3; i++) {
            char filename[256];
            snprintf(filename, sizeof(filename), "shard_0/00_%010d.tsm", i);
        ...
        co_await compactedTSM->readIndex();
        
        TSMResult<double> result(0);
        co_await compactedTSM->readSeries<double>("temperature.sensor1", 0, UINT64_MAX, result);
...
TEST_F(TSMCompactorTest, NewerValuesOverwriteOlderDuringCompaction) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        {
            char filename[256];
            snprintf(filename, sizeof(filename), "shard_0/00_0000000000.tsm");
            ...
            writer.writeSeries(TSMValueType::Float, "temperature.room1", timestamps, values);
            writer.writeIndex();
            writer.close();
        ...
        {
            char filename[256];
            snprintf(filename, sizeof(filename), "shard_0/00_0000000001.tsm");
        ...
        {
            char filename[256];
            snprintf(filename, sizeof(filename), "shard_0/00_0000000002.tsm");
        ...
        TSMResult<double> result(0);
        co_await compactedTSM->readSeries<double>("temperature.room1", 0, UINT64_MAX, result);
...
TEST_F(TSMCompactorTest, ReferenceCountingPreventsDelete) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        // Create a TSM file
        auto tsm = createTestTSMFile(0, 1, "data.", 2, 50);
        co_await tsm->open();
        co_await tsm->readIndex();
        ...
        // Simulate an active reader
        {
            ...
            EXPECT_TRUE(fs::exists("shard_0/00_0000000001.tsm"));
...
TEST_F(TSMCompactorTest, MultiLevelCompactionPreservesNewerValues) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        for (int fileNum = 0; fileNum < 4; fileNum++) {
            char filename[256];
            snprintf(filename, sizeof(filename), "shard_0/0_%d.tsm", fileNum);
            
            TSMWriter writer(filename);
            std::vector<uint64_t> timestamps;
            std::vector<double> values;
            
            // Each file has 5 timestamps, with overlap at timestamp 5000
            for (int i = 0; i < 5; i++) {
                uint64_t ts = (fileNum * 3000) + (i * 1000);
                // Special case: all files write to timestamp 5000
                if (ts == 5000 || i == 2) {
                    ts = 5000;
                }
                timestamps.push_back(ts);
                // Value encodes file number for tracking: fileNum * 100 + i
                values.push_back(fileNum * 100.0 + i);
            }
            
            writer.writeSeries(TSMValueType::Float, "metric.test", timestamps, values);
            writer.writeIndex();
            writer.close();
        ...
        for (int fileNum = 1; fileNum < 4; fileNum++) {
            char filename[256];
            snprintf(filename, sizeof(filename), "shard_0/1_%d.tsm", fileNum + 10);
        ...
        co_await level2TSM->readSeries<double>("metric.test", 0, UINT64_MAX, allData);
...
TEST_F(TSMCompactorTest, CompactionPlanGeneration) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        for (int tier = 0; tier < 3; tier++) {
            for (int i = 0; i < 5; i++) {
                auto tsm = createTestTSMFile(tier, tier * 10 + i, "metrics.", 3, 20);
                co_await tsm->open();
                co_await tsm->readIndex();
                fileManager->sequencedTsmFiles[tier * 10 + i] = tsm;
            }
        }
        
        // Generate compaction plan for tier 0
        auto plan = compactor->planCompaction(0);
...
TEST_F(TSMCompactorTest, ConcurrentReadsDuringCompaction) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        std::vector<seastar::shared_ptr<TSM>> files;
        for (int i = 0; i < 4; i++) {
            auto tsm = createTestTSMFile(0, i, "concurrent.", 5, 100);
            co_await tsm->open();
            co_await tsm->readIndex();
            files.push_back(tsm);
            fileManager->sequencedTsmFiles[i] = tsm;
        ...
        auto compactionTask = seastar::async([this, &files, &compactionDone]() {
            auto result = compactor->compact(files).get();
            compactionDone = true;
        ...
        for (int r = 0; r < 10; r++) {
            auto readTask = seastar::async([&files, &readsDone, r]() {
                ...
                std::string seriesKey = "concurrent.0";
...
TEST_F(TSMCompactorTest, CompactionStatistics) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            writer.writeSeries(TSMValueType::Float, "stats.test", timestamps, values);
            writer.writeIndex();
            writer.close();
        ...
        plan.targetPath = "shard_0/01_0000000100.tsm";
        ...
        EXPECT_EQ(stats.duplicatesRemoved, 100); // 50 points * 2 duplicates
...
TEST_F(TSMCompactorTest, FullCompaction) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        for (int tier = 0; tier < 3; tier++) {
            for (int i = 0; i < 3; i++) {
                auto tsm = createTestTSMFile(tier, totalFiles, "full.", 2, 10);
                co_await tsm->open();
                co_await tsm->readIndex();
                fileManager->sequencedTsmFiles[totalFiles] = tsm;
                totalFiles++;
            }
        }
        
        // Verify initial file count
        EXPECT_EQ(fileManager->sequencedTsmFiles.size(), 9);
        
        // Run full compaction
        co_await compactor->forceFullCompaction();
        
        // Should have fewer files after compaction
        // Each tier should be compacted
        EXPECT_LT(fileManager->sequencedTsmFiles.size(), 9);
...
TEST_F(TSMCompactorTest, MixedDataTypeCompaction) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            writer.writeSeries(TSMValueType::Boolean, "status", boolTimestamps, boolValues);
        ...
        auto keys = compactedTSM->getSeriesKeys();
...
TEST_F(TSMCompactorTest, CompactionErrorHandling) {
    seastar::app_template app;
    char* argv[] = {(char*)"test"};
    ...
    EXPECT_EQ(app.run(1, argv, [this]() -> seastar::future<int> {
        ...
        auto result = co_await compactor->compact(emptyFiles);
        EXPECT_TRUE(result.empty());
        
        // Test with invalid plan
        CompactionPlan invalidPlan;
        EXPECT_FALSE(invalidPlan.isValid());
        auto stats = co_await compactor->executeCompaction(invalidPlan);
