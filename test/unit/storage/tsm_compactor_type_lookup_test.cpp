...
TEST_F(TSMCompactorTypeLookupTest, TypeLookupChecksAllFiles) {
    ...
    EXPECT_TRUE(hasFileLoop) << "The series type lookup in compact() must iterate over all files, "
...
TEST_F(TSMCompactorTypeLookupTest, NoSingleFileTypeLookup) {
    ...
    EXPECT_FALSE(hasSingleFileLookup) << "Found 'files[0]->getSeriesType' in compact() series categorization. "
    ...
    EXPECT_TRUE(hasLoopVariableLookup) << "Expected 'file->getSeriesType' (using loop variable) in the series "
...
TEST_F(TSMCompactorTypeLookupTest, TypeLookupBreaksOnFirstMatch) {
    ...
    EXPECT_TRUE(hasBreak) << "The inner file loop should contain a 'break' statement after finding "
    ...
    EXPECT_TRUE(hasValueCheck) << "The inner file loop should check has_value() before breaking.\n"
                               << "Inner loop contents:\n"
                               << innerLoop;
}
...
TEST_F(TSMCompactorTypeLookupTest, AllSeriesIdsFromAllFiles) {
    ...
    EXPECT_TRUE(hasFileLoop) << "getAllSeriesIds() must iterate over all files to collect series IDs.\n"
                             << "Method contents:\n"
                             << methodBody;

    // Should use getSeriesIds()
    bool getsIds = (methodBody.find("getSeriesIds()") != std::string::npos);
    EXPECT_TRUE(getsIds) << "getAllSeriesIds() must call getSeriesIds() on each file.\n"
                         << "Method contents:\n"
                         << methodBody;

    // Should use a set or similar to deduplicate
    bool usesSet =
        (methodBody.find("set<") != std::string::npos || methodBody.find("unordered_set<") != std::string::npos);
    EXPECT_TRUE(usesSet) << "getAllSeriesIds() should use a set to deduplicate series IDs.\n"
                         << "Method contents:\n"
                         << methodBody;
}
