    ...
    // Test 4: Check bit width requirements
    {
        ...
        for (const auto& [selector, max_val] : max_values) {
            if (problematic_timestamp <= max_val) {
                std::cout << "Can be encoded with selector " << selector << " (max value: " << max_val << ")"
                          << std::endl;
                can_encode = true;
                break;
            }
        ...
        if (!can_encode) {
            ...
            std::cout << "Maximum encodable value is " << ((1ULL << 60) - 1) << " but timestamp is "
                      << problematic_timestamp << std::endl;
        }
    }
}
