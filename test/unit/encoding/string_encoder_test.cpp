...
protected:
    ...
    std::vector<std::string> generateRepetitiveStrings(size_t count) {
        ...
        std::vector<std::string> patterns = {"sensor_data_", "temperature_reading_", "status_update_", "error_message_",
                                             "warning_level_"};
...
TEST_F(StringEncoderTest, BasicEncodeDecodeTest) {
    std::vector<std::string> testStrings = {"hello", "world", "this is a test", "TSM string encoding",
...
TEST_F(StringEncoderTest, EmptyStringsTest) {
    std::vector<std::string> testStrings = {"", "not empty", "", "", "another string", ""};
...
TEST_F(StringEncoderTest, SpecialCharactersTest) {
    std::vector<std::string> testStrings = {"Hello\nWorld",
                                            "Tab\tSeparated",
...
TEST_F(StringEncoderTest, PartialDecodeTest) {
    std::vector<std::string> testStrings = {"first", "second", "third", "fourth", "fifth"};
...
TEST_F(StringEncoderTest, StringWithNullBytes) {
    ...
    ASSERT_EQ(decoded.size(), testStrings.size());
    for (size_t i = 0; i < testStrings.size(); i++) {
        EXPECT_EQ(decoded[i].size(), testStrings[i].size()) << "Size mismatch at index " << i;
        EXPECT_EQ(decoded[i], testStrings[i]);
    }
}
