...
TEST_F(ReplicatedVShardHostTest, ReceiverBoundsQuorumWaits) {
    seastar::async([] {
        ScopedShardedEngine eng;
        eng.start();
        cluster::EngineLocalStore store(*eng);
        ...
        // A caller without a deadline still gets the receiver's bounded wait.
        bool timedOut = false;
        try {
        ...
        EXPECT_TRUE(inventoryCounts->hasUnappliedEntries)
            << "the host must expose the ambiguous uncommitted tail after timeout";
