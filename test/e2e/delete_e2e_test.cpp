...
// Helper class to manage Engine lifecycle in tests
...
private:
    void cleanupTestDirectories() {
        // Remove shard directories created during tests
        for (int i = 0; i < 8; ++i) {
...
protected:
    std::unique_ptr<EngineTestHelper> helper;
    
    void SetUp() override {
        helper = std::make_unique<EngineTestHelper>();
    }
    ...
    bool isQueryResultEmpty(const VariantQueryResult& result) {
    ...
    void verifyQueryResult(const VariantQueryResult& result, 
