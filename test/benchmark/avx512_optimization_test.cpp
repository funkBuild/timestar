...
int main() {
    ...
    std::cout << std::setw(20) << "Implementation" << std::setw(15) << "Time (ms)" << std::setw(15) << "Throughput"
    ...
    std::cout << std::setw(20) << "Original AVX-512" << std::setw(15) << original_time << std::setw(12)
              << original_throughput << " MB/s" << std::setw(10) << "1.00x" << std::endl;

    std::cout << std::setw(20) << "Optimized AVX-512" << std::setw(15) << optimized_time << std::setw(12)
              << optimized_throughput << " MB/s" << std::setw(8) << std::setprecision(2)
              << (original_time / optimized_time) << "x" << std::endl;

    if (FloatEncoderAVX512Optimized::hasAVX512CD() && ultra_time > 0) {
        double ultra_throughput = (bytes / (1024.0 * 1024.0)) / (ultra_time / 1000.0);
        std::cout << std::setw(20) << "Ultra AVX-512" << std::setw(15) << ultra_time << std::setw(12)
                  << ultra_throughput << " MB/s" << std::setw(8) << std::setprecision(2) << (original_time / ultra_time)
                  << "x" << std::endl;
    }

    std::cout << "\nOptimization Impact: " << std::setprecision(1) << ((original_time / optimized_time - 1) * 100)
