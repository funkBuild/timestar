...
void printHeader() {
    ...
    std::cout << std::setw(30) << std::left << "Implementation"
...
void printResult(const BenchmarkResult& r) {
    if (!r.available) {
        std::cout << std::setw(30) << std::left << r.name
                  ...
                  << std::setw(12) << "N/A"
                  << std::setw(10) << "N/A"
                  << std::setw(15) << "Not Available" << std::endl;
        return;
    }
    
    std::cout << std::setw(30) << std::left << r.name
              << std::setw(12) << std::right << std::fixed << std::setprecision(3) << r.time_ms
              << std::setw(12) << std::setprecision(1) << r.throughput_mbps << " MB/s"
              << std::setw(12) << std::setprecision(2) << (r.compressed_size / 1024.0)
...
void runBenchmark(const std::string& dataset_name, const std::vector<double>& data) {
    ...
    if (best != results.end() && best->available && best->speedup > 1.0) {
        std::cout << "\nBest performer: " << best->name 
                  << " (" << std::setprecision(2) << best->speedup << "x speedup, "
                  << std::setprecision(1) << best->throughput_mbps << " MB/s)" << std::endl;
    }
...
int main() {
    ...
    std::cout << "  AVX-512:   " << (FloatEncoderAVX512::isAvailable() ? "✓ Available" : "✗ Not Available") << std::endl;
