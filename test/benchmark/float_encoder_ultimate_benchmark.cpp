...
void printResult(const BenchmarkResult& result, double compression_ratio) {
    ...
    std::cout << "| " << std::left << std::setw(25) << result.name
              << " | " << std::right << std::setw(10) << std::fixed << std::setprecision(3) << result.time_ms
...
void benchmarkDataset(const std::string& dataset_name, const std::vector<double>& data) {
    ...
    if (best != results.end() && best->available) {
        std::cout << "  Best performer: " << GREEN << best->name 
...
void runScalabilityTest() {
    ...
    std::cout << "\n" << BOLD << std::setw(12) << "Size" 
              ...
              << std::setw(15) << "AVX512 Speedup" << RESET << std::endl;
    ...
    for (size_t size : sizes) {
        ...
        std::cout << std::setw(12) << size
                  << std::setw(15) << std::fixed << std::setprecision(3) << original_ms << " ms";
        
        if (FloatEncoderSIMD::isAvailable()) {
            std::cout << std::setw(15) << avx2_ms << " ms"
...
int main() {
    ...
    std::cout << "  AVX2:    " << (FloatEncoderSIMD::isAvailable() ? 
                                    GREEN "✓ Available" RESET : 
                                    RED "✗ Not Available" RESET) << std::endl;
    std::cout << "  AVX-512: " << (FloatEncoderAVX512::isAvailable() ? 
                                    GREEN "✓ Available" RESET : 
                                    RED "✗ Not Available" RESET) << std::endl;
