...
protected:
    ...
    SeriesResult createMockSeries(
        ...
        for (int i = 0; i < numPoints; ++i) {
            ...
            if (i % 5 == 0) value += 2.0;  // Spike every 5 points
            if (i % 7 == 0) value -= 1.0;  // Dip every 7 points
...
TEST_F(QueryIntegrationTest, MetricsQueryWithAggregation) {
    ...
    mockData.push_back(createMockSeries(
        "cpu", 
        {{"host", "server01"}, {"datacenter", "us-west"}},
        "usage_percent", 60, 45.0, 0.3
    ));
    
    mockData.push_back(createMockSeries(
        "cpu",
        {{"host", "server02"}, {"datacenter", "us-west"}},
        "usage_percent", 60, 50.0, 0.4
    ));
    
    mockData.push_back(createMockSeries(
        "cpu",
        {{"host", "server03"}, {"datacenter", "us-east"}},
        "usage_percent", 60, 40.0, 0.25
    ));
...
TEST_F(QueryIntegrationTest, MaxTemperatureQuery) {
    ...
    sensorData.push_back(createMockSeries(
        "temperature",
        {{"room", "server_room"}, {"building", "A"}},
        "celsius", 120, 18.0, 0.1  // Server room: 18-30°C
    ));
    
    sensorData.push_back(createMockSeries(
        "temperature",
        {{"room", "office"}, {"building", "A"}},
        "celsius", 120, 20.0, 0.05  // Office: 20-26°C
    ));
    
    sensorData.push_back(createMockSeries(
        "temperature",
        {{"room", "storage"}, {"building", "B"}},
        "celsius", 120, 15.0, 0.08  // Storage: 15-25°C
    ));
...
TEST_F(QueryIntegrationTest, ComplexAggregationPipeline) {
    ...
    for (const auto& service : services) {
        for (const auto& region : regions) {
            responseTimeData.push_back(createMockSeries(
                "response_time",
...
TEST_F(QueryIntegrationTest, HandleEmptyAndSparseData) {
    ...
    auto emptyResult = Aggregator::aggregate(
        emptyTimestamps, emptyValues, AggregationMethod::AVG, interval);
...
TEST_F(QueryIntegrationTest, QueryParsingAndValidation) {
    ...
    std::vector<std::string> validQueries = {
        "avg:cpu(usage)",
        "max:memory(used,free){host:server*}",
    ...
    for (const auto& query : validQueries) {
        try {
            ...
            FAIL() << "Failed to parse valid query: " << query 
                   << " Error: " << e.what();
        }
    }
...
TEST_F(QueryIntegrationTest, SeriesMatchingWithPatterns) {
    ...
    std::vector<TestCase> testCases = {
        ...
        {
            {{"host", "web-*"}, {"datacenter", "us-*"}}, 
            true, 
            "Multiple wildcards match"
        },
        {
            {{"host", "web-*"}, {"datacenter", "eu-*"}}, 
            false, 
            "One wildcard doesn't match"
        }
    };
...
TEST_F(QueryIntegrationTest, PerformanceWithLargeDataset) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        largeDataset.push_back(createMockSeries(
            "metric",
    ...
    EXPECT_LT(duration.count(), 1000) 
        << "Aggregation took too long: " << duration.count() << "ms";
}