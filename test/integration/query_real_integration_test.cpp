...
TEST_F(QueryRealIntegrationTest, RealDataAggregation) {
    ...
    for (const auto& [key, data] : allData) {
        ...
        auto aggregated = Aggregator::aggregate(data.timestamps, data.values, AggregationMethod::AVG, interval);
        ...
        auto maxAggregated = Aggregator::aggregate(data.timestamps, data.values, AggregationMethod::MAX, interval);
...
TEST_F(QueryRealIntegrationTest, SeriesMatcherRealPatterns) {
    ...
    std::map<std::string, std::string> webProdQuery = {{"host", "prod-web-*"}, {"env", "production"}};
    ...
    std::map<std::string, std::string> usWestQuery = {{"dc", "us-west-*"}};
    ...
    std::map<std::string, std::string> prodDbQuery = {{"host", "*-db-*"}, {"env", "production"}};
...
TEST_F(QueryRealIntegrationTest, CompleteQueryPipeline) {
    ...
    for (const auto& host : hosts) {
        for (const auto& iface : interfaces) {
            ...
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_real_distribution<> dis(iface == "lo" ? 100.0 : 10000.0,  // Loopback has less traffic
        ...
        std::visit(
            [&](auto&& series) {
                ...
                if constexpr (std::is_same_v<T, InMemorySeries<double>>) {
                    ...
                    auto aggregated =
                        Aggregator::aggregate(series.timestamps, series.values, AggregationMethod::MAX, interval);
...
TEST_F(QueryRealIntegrationTest, RealDataEdgeCases) {
    ...
    auto aggregated = Aggregator::aggregate(sparseTimestamps, sparseValues, AggregationMethod::AVG, interval);
    ...
    auto denseAggregated = Aggregator::aggregate(denseTimestamps, denseValues, AggregationMethod::AVG, secondInterval);
    ...
    std::vector<uint64_t> dupTimestamps = {startTime, startTime + 1000000000ULL, startTime + 2000000000ULL};
...
TEST_F(QueryRealIntegrationTest, PerformanceWithRealisticData) {
    ...
    EXPECT_LT(insertDuration.count(), 5000) << "Insertion too slow: " << insertDuration.count() << "ms";
    ...
    auto queryDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endQuery - startQuery);

    EXPECT_EQ(totalPoints, 100000);
    EXPECT_LT(queryDuration.count(), 1000) << "Query too slow: " << queryDuration.count() << "ms";
    ...
    for (const auto& [key, variantSeries] : memoryStore->series) {
        if (seriesProcessed >= 10)
            break;  // Test first 10 series
        ...
        std::visit(
            [&](auto&& series) {
                ...
                if constexpr (std::is_same_v<T, InMemorySeries<double>>) {
                    ...
                    auto aggregated =
                        Aggregator::aggregate(series.timestamps, series.values, AggregationMethod::AVG, interval);
    ...
    auto aggDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endAgg - startAgg);
