...
static std::vector<QueryDef> buildQuerySuite(WireFormat format = WireFormat::Json) {
    ...
    qs.push_back(
        {"latest: single field", "POST", "/query", q("latest:server.metrics(cpu_usage)", BASE_TS, END_TS), 20});

    qs.push_back({"latest: all fields", "POST", "/query", q("latest:server.metrics()", BASE_TS, END_TS), 20});

    qs.push_back({"latest: single field, tag filter", "POST", "/query",
                  q("latest:server.metrics(cpu_usage){host:host-01}", BASE_TS, END_TS), 20});

    qs.push_back({"latest: all fields, two tag filters", "POST", "/query",
                  q("latest:server.metrics(){host:host-01,rack:rack-1}", BASE_TS, END_TS), 20});

    // ── 2. Aggregation methods (narrow range) ───────────────────────

    qs.push_back({"avg: narrow range, single field", "POST", "/query",
                  q("avg:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 20});

    qs.push_back({"min: narrow range, single field", "POST", "/query",
                  q("min:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 20});

    qs.push_back({"max: narrow range, single field", "POST", "/query",
                  q("max:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 20});

    qs.push_back({"sum: narrow range, single field", "POST", "/query",
                  q("sum:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 20});

    qs.push_back({"count: narrow range, single field", "POST", "/query",
                  q("count:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 20});

    qs.push_back({"median: narrow range, single field", "POST", "/query",
                  q("median:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 10});

    qs.push_back({"stddev: narrow range, single field", "POST", "/query",
                  q("stddev:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 10});

    // ── 3. Field cardinality ────────────────────────────────────────

    qs.push_back(
        {"avg: narrow, all 10 fields", "POST", "/query", q("avg:server.metrics()", NARROW_START, NARROW_END), 10});

    qs.push_back({"avg: narrow, 3 fields", "POST", "/query",
                  q("avg:server.metrics(cpu_usage,memory_usage,temperature)", NARROW_START, NARROW_END), 10});

    // ── 4. Time range scaling ───────────────────────────────────────

    qs.push_back({"avg: medium range (~7d), single field", "POST", "/query",
                  q("avg:server.metrics(cpu_usage)", MED_START, MED_END), 10});

    qs.push_back(
        {"avg: full range, single field", "POST", "/query", q("avg:server.metrics(cpu_usage)", BASE_TS, END_TS), 5});

    qs.push_back({"avg: full range, all fields", "POST", "/query", q("avg:server.metrics()", BASE_TS, END_TS), 5});

    // ── 5. Tag-filtered queries (scopes) ────────────────────────────

    qs.push_back({"avg: single host filter", "POST", "/query",
                  q("avg:server.metrics(cpu_usage){host:host-01}", NARROW_START, NARROW_END), 20});

    qs.push_back({"avg: host + rack filter", "POST", "/query",
                  q("avg:server.metrics(cpu_usage){host:host-01,rack:rack-1}", NARROW_START, NARROW_END), 20});

    qs.push_back({"avg: rack filter, all fields", "POST", "/query",
                  q("avg:server.metrics(){rack:rack-2}", NARROW_START, NARROW_END), 10});

    // ── 6. Group-by queries ─────────────────────────────────────────

    qs.push_back({"avg: group by host", "POST", "/query",
                  q("avg:server.metrics(cpu_usage) by {host}", NARROW_START, NARROW_END), 10});

    qs.push_back({"avg: group by rack", "POST", "/query",
                  q("avg:server.metrics(cpu_usage) by {rack}", NARROW_START, NARROW_END), 10});

    qs.push_back({"avg: group by host+rack", "POST", "/query",
                  q("avg:server.metrics(cpu_usage) by {host,rack}", NARROW_START, NARROW_END), 10});

    qs.push_back({"avg: group by host, full range", "POST", "/query",
                  q("avg:server.metrics(cpu_usage) by {host}", BASE_TS, END_TS), 5});

    // ── 7. Time-bucketed aggregation ────────────────────────────────

    qs.push_back({"avg: 1h buckets, medium range", "POST", "/query",
                  q("avg:server.metrics(cpu_usage)", MED_START, MED_END, "1h"), 10});

    qs.push_back({"avg: 5m buckets, narrow range", "POST", "/query",
                  q("avg:server.metrics(cpu_usage)", NARROW_START, NARROW_END, "5m"), 20});

    qs.push_back({"avg: 1d buckets, full range", "POST", "/query",
                  q("avg:server.metrics(cpu_usage)", BASE_TS, END_TS, "1d"), 5});

    qs.push_back({"max: 1h buckets, tag filter", "POST", "/query",
                  q("max:server.metrics(cpu_usage){host:host-05}", MED_START, MED_END, "1h"), 10});

    qs.push_back({"avg: 1h buckets, group by host", "POST", "/query",
                  q("avg:server.metrics(cpu_usage) by {host}", MED_START, MED_END, "1h"), 5});

    qs.push_back({"avg: 5m buckets, all fields, tag filter", "POST", "/query",
                  q("avg:server.metrics(){rack:rack-1}", MED_START, MED_END, "5m"), 5});

    // ── 8. Metadata queries ─────────────────────────────────────────

    qs.push_back({"GET /measurements", "GET", "/measurements", "", 20});

    qs.push_back({"GET /tags?measurement=server.metrics", "GET", "/tags?measurement=server.metrics", "", 20});

    qs.push_back({"GET /fields?measurement=server.metrics", "GET", "/fields?measurement=server.metrics", "", 20});

    // ── 9. Stress patterns ──────────────────────────────────────────

    qs.push_back({"avg: narrow, single field (repeated)", "POST", "/query",
                  q("avg:server.metrics(cpu_usage)", NARROW_START, NARROW_END), 50});
...
static future<QueryResult> runQuery(socket_address addr, const QueryDef& def) {
    ...
    for (int i = 0; i < def.iterations; ++i) {
        ...
        try {
            co_await client->make_request(
                std::move(req), [&ok, &respBody](const http::reply& rep, input_stream<char>&& body_in) -> future<> {
                    auto body = std::move(body_in);
                    ok = (static_cast<int>(rep._status) >= 200 && static_cast<int>(rep._status) < 300);
                    sstring acc;
        ...
        if (ok) {
            ...
            result.failures++;
            if (result.first_error.empty()) {
                result.first_error = std::string(respBody.data(), std::min(respBody.size(), size_t(256)));
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    return app.run(argc, argv, [&]() -> future<> {
        ...
        // ── Health check ───────────────────────────────────────────
        {
            ...
            try {
                co_await hclient->make_request(
                    std::move(req), [&healthy](const http::reply& rep, input_stream<char>&& body_in) -> future<> {
                        auto body = std::move(body_in);
        ...
        // Apply filter
        if (!filter.empty()) {
            std::erase_if(suite, [&](const QueryDef& d) { return d.name.find(filter) == std::string::npos; });
        ...
        for (size_t qi = 0; qi < suite.size(); ++qi) {
            auto& def = suite[qi];
            fmt::print("[{}/{}] {} ({}x) ... ", qi + 1, suite.size(), def.name, def.iterations);
        ...
        fmt::print(
            ...
            "{:.1f}ms total\n",
            results.size(), totalOk + totalFail, totalOk, totalFail, totalBenchMs);
