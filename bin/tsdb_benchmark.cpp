...
seastar::future<> run_benchmark(seastar::sharded<Engine>& engine) {
    ...
    for (size_t i = 0; i < NUM_INSERTS; ++i) {
        ...
        co_await engine.invoke_on(targetShard, [insert](Engine& e) {
            return e.insert(insert);
        });
    ...
    for (size_t i = 0; i < NUM_QUERIES; ++i) {
        ...
        auto opStart = std::chrono::high_resolution_clock::now();
        auto result = co_await engine.invoke_on(targetShard, 
            [seriesKey, startTime, endTime](Engine& e) {
                return e.query(seriesKey, startTime, endTime);
            });
        auto opEnd = std::chrono::high_resolution_clock::now();
    ...
    std::cout << "Disk usage:      " << std::fixed << std::setprecision(2) 
              << (totalDiskUsage / 1024.0 / 1024.0) << " MB" << std::endl;
