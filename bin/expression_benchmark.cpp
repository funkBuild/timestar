...
struct GlobalSetup {
    GlobalSetup() {
        ...
        for (size_t i = 0; i < N; ++i) {
            ...
            vs[i] = 100.0 + 50.0 * std::sin(static_cast<double>(i) * 0.01) + 0.1 * static_cast<double>(i % 1000) +
        ...
        std::srand(99);
        for (size_t i = 0; i < N; ++i) {
            vs2[i] = 80.0 + 40.0 * std::sin(static_cast<double>(i) * 0.01 + 1.0) +
                     0.05 * static_cast<double>(i % 1000) + static_cast<double>(std::rand() % 8);
            if (i % 10000 == 0) {
        ...
        for (size_t g = 0; g < NUM_GROUPS; ++g) {
            ...
            for (size_t i = 0; i < GROUP_N; ++i) {
                ...
                gvs[i] = static_cast<double>(g * 20 + 50) +
                         30.0 * std::sin(static_cast<double>(i) * 0.02 + static_cast<double>(g) * 0.5) +
