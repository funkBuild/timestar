...
struct glz::meta<timestar::cluster::NodeIdentity> {
    ...
    static constexpr auto value =
        object("node_uuid", &T::node_uuid, "cluster_uuid", &T::cluster_uuid);
};
...
NodeIdentity NodeIdentity::loadOrCreate(const std::filesystem::path& dataDir) {
    ...
    if (std::filesystem::exists(path, ec) && !ec) {
        ...
        if (in) {
            ...
            if (err)
                throw std::runtime_error("node.json is present but unparseable at " +
                                         path.string() + " (refusing to overwrite an existing identity)");
...
void NodeIdentity::persist(const std::filesystem::path& dataDir) const {
    ...
    if (fd < 0)
        throw std::runtime_error("failed to open node.json.tmp for write: " + tmp + ": " +
                                 std::strerror(errno));
    size_t written = 0;
    while (written < out.size()) {
        ...
        if (n < 0) {
            int e = errno;
            ::close(fd);
            throw std::runtime_error("failed to write node.json.tmp: " + tmp + ": " +
                                     std::strerror(e));
        }
