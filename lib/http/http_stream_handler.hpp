...
public:
    explicit HttpStreamHandler(seastar::sharded<Engine>* engine)
        : _engineSharded(engine) {}
    ...
    handleSubscribe(std::unique_ptr<seastar::http::request> req);

    seastar::future<std::unique_ptr<seastar::http::reply>>
    handleGetSubscriptions(std::unique_ptr<seastar::http::request> req);
    ...
    static std::vector<StreamingBatch> queryResponseToBatches(
    ...
    static StreamingBatch applyFormulaToBatch(
