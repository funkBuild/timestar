...
struct SortedSpan {
  ...
  void advancePast(uint64_t target) {
    while (pos < len && tsPtr[pos] <= target) ++pos;
  }
};
...
template <class T>
static void mergeTwoSpans(
    SortedSpan<T>& a, SortedSpan<T>& b,
    std::vector<uint64_t>& outTs, std::vector<T>& outVal)
{
    ...
    std::vector<uint64_t>& outTs, std::vector<T>& outVal)
{
  ...
  for (size_t i = 0; i < K; ++i) {
    if (spans[i].exhausted()) --activeCount;
  }
...
seastar::future<QueryResult<T>> QueryRunner::queryTsm(const std::string& series, const SeriesId128& seriesId, uint64_t startTime, uint64_t endTime){
  ...
  for (const auto* memoryData : memoryMatches) {
    ...
    auto beginIt = std::lower_bound(storeData.timestamps.begin(),
                                    storeData.timestamps.end(),
                                    startTime);
    auto endIt = std::upper_bound(beginIt,
                                  storeData.timestamps.end(),
                                  endTime);
    ...
    uint64_t aggregationInterval)
{
    ...
    auto memMinTimeOpt = walFileManager->getEarliestMemoryTimestamp<double>(
    ...
    if (needsFallback) {
        QueryResult<double> fallbackResult = co_await queryTsm<double>(
            seriesKey, seriesId, fallbackStartTime, endTime);
