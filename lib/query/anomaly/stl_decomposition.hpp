...
public:
    ...
    static STLComponents decompose(
...
private:
    ...
    static std::vector<double> loess(
    ...
    static std::vector<double> computeSeasonalMeans(
        const std::vector<double>& values,
        size_t period
    );

    // Robustness weights based on residuals
    static std::vector<double> computeRobustnessWeights(
