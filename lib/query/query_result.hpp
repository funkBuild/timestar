...
public:
    ...
    void mergeTwoWay(std::vector<TSMIterationState>& iters, std::vector<TSMResult<T>>& tsmResults) {
        auto& s0 = iters[0];
        auto& s1 = iters[1];
    ...
    void mergeSmallN(std::vector<TSMIterationState>& iters, const size_t numIters,
                     std::vector<TSMResult<T>>& tsmResults) {
    ...
    void mergeTsmResults(std::vector<TSMResult<T>>& tsmResults) {
        ...
        for (size_t i = 0; i < tsmResultSize; i++) {
            ...
            TSMIterationState state = {i, firstBlock, 0, 0, firstBlock->size(), 0, tsmResults[i].rank};
        ...
        for (size_t i = 0; i < blockIterState.size(); i++) {
            heap.push({blockIterState[i].currentTimestamp, blockIterState[i].rank, i});
        }
