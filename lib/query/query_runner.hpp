...
private:
  ...
  template <class T>
  seastar::future<QueryResult<T>> queryTsm(std::string series, SeriesId128 seriesId, uint64_t startTime, uint64_t endTime);
public:
  ...
  seastar::future<VariantQueryResult> runQuery(std::string series, SeriesId128 seriesId, uint64_t startTime, uint64_t endTime);
