...
void TSMWriter::writeSeriesDirect(TSMValueType seriesType, const SeriesId128& seriesId,
    ...
    if (timestamps.size() > MaxPointsPerBlock()) {
        LOG_INSERT_PATH(timestar::tsm_log, debug,
                        "Series has {} points (>{}), writing as multiple blocks with fallback", timestamps.size(),
                        MaxPointsPerBlock());
...
template <class T>
void TSMWriter::writeBlockDirect(TSMValueType seriesType, const SeriesId128& seriesId,
                                 std::vector<uint64_t>&& timestamps, std::vector<T>&& values,
                                 TSMIndexEntry& indexEntry) {
...
template void TSMWriter::writeSeriesDirect<double>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                   std::vector<uint64_t>&& timestamps, std::vector<double>&& values);
template void TSMWriter::writeSeriesDirect<bool>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                 std::vector<uint64_t>&& timestamps, std::vector<bool>&& values);
template void TSMWriter::writeSeriesDirect<std::string>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                        std::vector<uint64_t>&& timestamps,
                                                        std::vector<std::string>&& values);
template void TSMWriter::writeSeriesDirect<int64_t>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                    std::vector<uint64_t>&& timestamps, std::vector<int64_t>&& values);

template void TSMWriter::writeBlockDirect<double>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                  std::vector<uint64_t>&& timestamps, std::vector<double>&& values,
                                                  TSMIndexEntry& indexEntry);
template void TSMWriter::writeBlockDirect<bool>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                std::vector<uint64_t>&& timestamps, std::vector<bool>&& values,
                                                TSMIndexEntry& indexEntry);
template void TSMWriter::writeBlockDirect<std::string>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                       std::vector<uint64_t>&& timestamps,
                                                       std::vector<std::string>&& values, TSMIndexEntry& indexEntry);
template void TSMWriter::writeBlockDirect<int64_t>(TSMValueType seriesType, const SeriesId128& seriesId,
                                                   std::vector<uint64_t>&& timestamps, std::vector<int64_t>&& values,
                                                   TSMIndexEntry& indexEntry);
