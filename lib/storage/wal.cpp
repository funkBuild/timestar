...
void WAL::encodeInsertEntry(AlignedBuffer& buffer, TimeStarInsert<T>& insertRequest) {
    ...
    if (!insertRequest.revisions.empty()) {
        ...
        buffer.write(static_cast<uint32_t>(0));  // placeholder for encoded size
        const size_t startPos = buffer.size();
        IntegerEncoder::encodeInto(insertRequest.revisions, buffer);
        buffer.writeAt(sizePos, static_cast<uint32_t>(buffer.size() - startPos));
    }
...
TimeStarInsert<T> WALReader::readSeries(Slice& walSlice, const std::string& seriesKey) {
    ...
    if (walSlice.bytesLeft() >= sizeof(uint32_t)) {
        ...
        if (encodedRevSize > MAX_ENCODED_SIZE) {
            timestar::wal_log.error("WAL recovery: Invalid revision column size {} for '{}'", encodedRevSize,
                                    seriesKey);
            throw std::runtime_error("Invalid revision column size in WAL entry");
        }
        auto revSlice = walSlice.getSlice(encodedRevSize);
        IntegerEncoder::decode(revSlice, timestampsCount, insertReq.revisions);
    }
