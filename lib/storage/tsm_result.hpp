...
public:
    ...
    uint64_t startTime() { return timestamps.front(); }

    uint64_t endTime() { return timestamps.back(); }

    size_t size() { return timestamps.size(); }

    uint64_t timestampAt(size_t idx) { return timestamps[idx]; }

    value_return_type valueAt(size_t idx) const { return values[idx]; }
};
