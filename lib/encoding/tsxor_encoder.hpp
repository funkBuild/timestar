...
public:
    ...
    inline bool contains(uint64_t val) { return std::find(buffer.begin(), buffer.end(), val) != buffer.end(); }

    inline int getIndexOf(uint64_t val) {
        return std::distance(buffer.begin(), std::find(buffer.begin(), buffer.end(), val));
    }

    inline uint64_t get(int offset) { return buffer[offset]; }

    inline uint64_t getLast() { return buffer.front(); }
    ...
    inline uint64_t getCandidate(uint64_t val) {
        ...
        std::transform(tmp.begin(), tmp.end(), tmp.begin(), [&](uint64_t x) {
            if (x != 0) {
