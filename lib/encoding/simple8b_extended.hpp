#ifndef __SIMPLE8B_EXTENDED_H_INCLUDED__
#define __SIMPLE8B_EXTENDED_H_INCLUDED__
...
private:
    ...
    static bool needsOverflow(uint64_t value) {
        return value > MAX_SIMPLE8B_VALUE;
    }
    ...
    static constexpr PackingScheme schemes[] = {
        {0, 240, 0},  // Special: 240 zeros
    ...
    static uint64_t pack(const std::vector<uint64_t>& values, size_t& offset,
                        uint64_t selector, uint64_t n, uint64_t bits);
    static void unpack(uint64_t packedValue, std::vector<uint64_t>& out,
                      uint64_t n, uint64_t bits);
};
