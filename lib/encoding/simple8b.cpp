...
std::vector<uint64_t> Simple8B::decode(Slice& encoded, unsigned int size) {
    ...
    for (unsigned int i = 0; i < length; i++) {
        ...
        if (selector > 15) {
            throw std::runtime_error("Simple8B decode: invalid selector value " + std::to_string(selector) +
                                     " at position " + std::to_string(i));
        }

        switch (selector) {
            case 0:
                throw std::runtime_error("Simple8B decode: unexpected selector 0 at position " + std::to_string(i) +
                                         " (encoder never produces selector 0, data may be corrupt)");
            case 1:
                throw std::runtime_error("Simple8B decode: unexpected selector 1 at position " + std::to_string(i) +
                                         " (encoder never produces selector 1, data may be corrupt)");
            case 2:
