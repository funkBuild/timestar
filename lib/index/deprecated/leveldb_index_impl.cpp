...
seastar::future<> LevelDBIndex::open() {
    ...
    if (shardId != 0) {
        timestar::index_log.info("Shard {} skipping LevelDB index opening - metadata centralized on shard 0", shardId);
        co_return;
    }

    timestar::index_log.info("Shard 0 opening centralized metadata LevelDB index");

    // Create index directory if it doesn't exist.
    // Wrap in seastar::async to avoid blocking the reactor thread.
    co_await seastar::async([this]() { std::filesystem::create_directories(indexPath); });

    const auto& idxCfg = timestar::config().index;
    ...
    if (!status.ok()) {
        timestar::index_log.error("Failed to open LevelDB index: {}", status.ToString());
        throw std::runtime_error("Failed to open LevelDB index: " + status.ToString());
    }

    db.reset(dbPtr);
    timestar::index_log.info("LevelDB index opened at: {}", indexPath);
...
bool LevelDBIndex::seriesCacheContains(const std::string& key) const {
    return indexedSeriesCache_.count(key) > 0 || indexedSeriesCacheRetired_.count(key) > 0;
}
...
// Escape delimiter characters in index key components to prevent collisions.
// Delimiters: comma (,), equals (=), space ( ), backslash (\).
static std::string escapeKeyComponent(const std::string& s) {
    // Fast path: most strings have no special chars. Check before allocating.
    ...
    std::string out;
    out.reserve(s.size() + s.size() / 4);  // ~25% overhead for escaped chars
    for (char c : s) {
        switch (c) {
            case '\\':
                out += "\\\\";
...
std::string LevelDBIndex::encodeSeriesKey(const std::string& measurement,
    ...
    // Pre-estimate total size to avoid repeated reallocations.
    ...
    // All components are escaped to prevent delimiter collisions.
    key += escapeKeyComponent(measurement);

    // Tags are from std::map, already sorted by key
...
std::string LevelDBIndex::encodeTagValuesKey(const std::string& measurement, const std::string& tagKey) {
    // Reject embedded null bytes — this key uses \0 as the field separator.
    if (measurement.find('\0') != std::string::npos || tagKey.find('\0') != std::string::npos) {
        throw std::invalid_argument("Measurement or tag key contains null byte, which is not allowed");
    }
    std::string key;
...
std::string LevelDBIndex::encodeStringSet(const std::set<std::string>& strings) {
    // Single-pass: estimate total size (assuming typical strings < 256 bytes)
    // and encode in one iteration. Reserve pessimistically; the result string
    // will shrink-to-fit on return via move.
    std::string result;
...
std::set<std::string> LevelDBIndex::decodeStringSet(const std::string& encoded) {
    ...
    while (offset + sizeof(uint32_t) <= size) {
        ...
        if (offset + len > size) {
            timestar::index_log.warn("decodeStringSet: truncated data at offset {} (len={}, remaining={})", offset, len,
                                     size - offset);
            break;
        }
...
std::string LevelDBIndex::encodeSeriesMetadata(const SeriesMetadata& metadata) {
    try {
        ...
        timestar::index_log.error("encodeSeriesMetadata failed: {}", e.what());
...
seastar::future<SeriesId128> LevelDBIndex::getOrCreateSeriesId(std::string measurement,
    ...
    if (shardId != 0) {
        throw std::runtime_error("getOrCreateSeriesId called on non-zero shard " + std::to_string(shardId) +
    ...
    // TimeStarInsert::seriesKey(), so external IDs cannot be reused here.
    ...
    if (seriesCacheContains(seriesKeyStr)) {
        ...
        LOG_INSERT_PATH(timestar::index_log, trace, "[INDEX] Cache hit for series: '{}'", seriesKeyStr);
        co_return seriesId;
    }
    ...
    LOG_INSERT_PATH(timestar::index_log, debug, "[INDEX] Cache miss for series: '{}', checking LevelDB", seriesKeyStr);
    ...
    if (existingMetadata.has_value()) {
        ...
        LOG_INSERT_PATH(timestar::index_log, debug, "[INDEX] Series found in LevelDB, added to cache: '{}'",
                        seriesKeyStr);
        co_return seriesId;
    }

    // Series doesn't exist - create it
    LOG_INSERT_PATH(timestar::index_log, debug, "[INDEX] Creating new series index: '{}'", seriesKeyStr);
    ...
    if (fieldsCacheMiss || tagsCacheMiss || !tvLoadsNeeded.empty()) {
        ...
        // Merge loaded data into caches. We must merge (not overwrite) because
        // concurrent coroutines may have already populated the cache with new
        // entries between our cache-miss check and this resume point.
        if (fieldsCacheMiss) {
            fieldsCache[measurement].insert(loaded.fields.begin(), loaded.fields.end());
        }
        if (tagsCacheMiss) {
            tagsCache[measurement].insert(loaded.tagKeys.begin(), loaded.tagKeys.end());
        }
        for (auto& [tvCacheKey, values] : loaded.tagValues) {
            tagValuesCache[tvCacheKey].insert(values.begin(), values.end());
        }
    }
    ...
    std::string encodedSeriesId = encodeSeriesId(seriesId);
    for (const auto& tag : tags) {
        const std::string& tagKey = tag.first;
        const std::string& tagValue = tag.second;

        std::string indexKey;
        indexKey.reserve(1 + measurement.size() + 1 + tagKey.size() + 1 + tagValue.size() + 1 + 16);
        indexKey.push_back(TAG_INDEX);
        indexKey.append(measurement);
        indexKey.push_back('\0');
        indexKey.append(tagKey);
        indexKey.push_back('\0');
        indexKey.append(tagValue);
        indexKey.push_back('\0');
        indexKey.append(encodedSeriesId);
        batch.Put(indexKey, encodedSeriesId);

        // Reuse the same key for GROUP_BY_INDEX: only the prefix byte differs
        indexKey[0] = GROUP_BY_INDEX;
        batch.Put(indexKey, encodedSeriesId);
        ...
        if (tagValuesCache[tvCacheKey].insert(tagValue).second) {
            batch.Put(encodeTagValuesKey(measurement, tagKey), encodeStringSet(tagValuesCache[tvCacheKey]));
        }
    }

    // MEASUREMENT_FIELDS: write the updated field set if it changed.
    if (fieldNeedsWrite) {
        batch.Put(encodeMeasurementFieldsKey(measurement), encodeStringSet(fieldsCache[measurement]));
    }

    // MEASUREMENT_TAGS: write the updated tag-keys set if it changed.
    if (tagsNeedWrite) {
        batch.Put(encodeMeasurementTagsKey(measurement), encodeStringSet(tagsCache[measurement]));
    }
    ...
    auto status = co_await seastar::async([this, &batch] { return db->Write(leveldb::WriteOptions(), &batch); });
    if (!status.ok()) {
...
seastar::future<> LevelDBIndex::addField(const std::string& measurement, const std::string& field) {
    // Metadata operations only supported on shard 0
    if (shardId != 0) {
        throw std::runtime_error("addField called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata operations only supported on shard 0");
    }
    ...
    if (cacheIt != fieldsCache.end()) {
        ...
        // Cache miss: load from LevelDB and merge into cache.
        // Must merge (not overwrite) because a concurrent coroutine may have
        // populated the cache between our find() and this resume point.
        auto fields = co_await getFields(measurement);
        auto& cached = fieldsCache[measurement];
        cached.insert(fields.begin(), fields.end());
        cached.insert(field);
    }
    ...
    auto status = co_await seastar::async(
        [this, &key, &encodedFields] { return db->Put(leveldb::WriteOptions(), key, encodedFields); });
...
seastar::future<> LevelDBIndex::addTag(const std::string& measurement, const std::string& tagKey,
                                       const std::string& tagValue) {
    // Metadata operations only supported on shard 0
    if (shardId != 0) {
        throw std::runtime_error("addTag called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata operations only supported on shard 0");
    }
    ...
    if (tagsCacheIt != tagsCache.end()) {
        ...
        auto& cached = tagsCache[measurement];
        cached.insert(tags.begin(), tags.end());
        tagsNeedWrite = (cached.count(tagKey) == 0);
        cached.insert(tagKey);
    }
    ...
    if (tvCacheIt != tagValuesCache.end()) {
        ...
        auto& cached = tagValuesCache[tagValuesCacheKey];
        cached.insert(tagValues.begin(), tagValues.end());
        tagValuesNeedWrite = (cached.count(tagValue) == 0);
        cached.insert(tagValue);
    }
...
seastar::future<> LevelDBIndex::addFieldsAndTags(const std::string& measurement, const std::string& field,
                                                 const std::map<std::string, std::string>& tags) {
    // Metadata operations only supported on shard 0
    if (shardId != 0) {
        throw std::runtime_error("addFieldsAndTags called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata operations only supported on shard 0");
    }
    if (!db) {
...
seastar::future<std::set<std::string>> LevelDBIndex::getAllMeasurements() {
    ...
    if (shardId != 0) {
        throw std::runtime_error("getAllMeasurements called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata queries only supported on shard 0");
    }
    ...
    auto measurements = co_await seastar::async([this] {
        ...
        // Use fill_cache=false for scans to avoid polluting the block cache
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
    ...
    timestar::index_log.debug("Found {} measurements in index", measurements.size());
...
seastar::future<> LevelDBIndex::setFieldType(const std::string& measurement, const std::string& field,
                                             const std::string& type) {
    // Only shard 0 manages metadata
    if (shardId != 0) {
        throw std::runtime_error("setFieldType called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata management only supported on shard 0");
    }
    ...
    // Offload blocking LevelDB I/O to Seastar thread pool
    auto status = co_await seastar::async([this, &key, &type] { return db->Put(leveldb::WriteOptions(), key, type); });
...
seastar::future<std::string> LevelDBIndex::getFieldType(const std::string& measurement, const std::string& field) {
    ...
    if (shardId != 0) {
        throw std::runtime_error("getFieldType called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata queries only supported on shard 0");
    }
...
seastar::future<std::set<std::string>> LevelDBIndex::getTags(const std::string& measurement) {
    // Metadata queries only supported on shard 0
    if (shardId != 0) {
        throw std::runtime_error("getTags called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata queries only supported on shard 0");
    }
...
seastar::future<std::set<std::string>> LevelDBIndex::getTagValues(const std::string& measurement,
                                                                  const std::string& tagKey) {
    // Metadata queries only supported on shard 0
    if (shardId != 0) {
        throw std::runtime_error("getTagValues called on non-zero shard " + std::to_string(shardId) +
                                 " - metadata queries only supported on shard 0");
    }
...
template <class T>
seastar::future<SeriesId128> LevelDBIndex::indexInsert(const TimeStarInsert<T>& insert) {
    // seriesKey() is const (uses mutable cache internally) -- no copy needed
    const std::string& seriesKeyStr = insert.seriesKey();

    LOG_INSERT_PATH(timestar::index_log, debug,
                    "[INDEX] indexInsert called for measurement: '{}', field: '{}', series key: '{}'",
    ...
    LOG_INSERT_PATH(timestar::index_log, debug, "[INDEX] indexInsert completed - series ID: {} for series: '{}'",
...
seastar::future<> LevelDBIndex::indexMetadataBatch(const std::vector<MetadataOp>& ops) {
    ...
    LOG_INSERT_PATH(timestar::index_log, debug, "[INDEX] indexMetadataBatch called with {} ops", ops.size());
    ...
    for (size_t i = 0; i < ops.size(); ++i) {
        ...
        if (seriesCacheContains(seriesKeyStr)) {
            ...
            LOG_INSERT_PATH(timestar::index_log, trace, "[INDEX] Batch cache hit for series: '{}'", seriesKeyStr);
        } else {
    ...
    if (!pending.empty()) {
        ...
        for (auto& p : pending) {
            if (!p.isNew) {
                ...
                LOG_INSERT_PATH(timestar::index_log, debug,
                                "[INDEX] Batch: series found in LevelDB, added to cache: '{}'", p.seriesKeyStr);
            }
        }
    }
    ...
    for (auto& p : pending) {
        ...
        // Encode series ID once; build one key and reuse for GROUP_BY by mutating prefix byte.
        std::string encodedId = encodeSeriesId(p.seriesId);
        for (const auto& [tagKey, tagValue] : op.tags) {
            std::string indexKey;
            indexKey.reserve(1 + op.measurement.size() + 1 + tagKey.size() + 1 + tagValue.size() + 1 + 16);
            indexKey.push_back(TAG_INDEX);
            indexKey.append(op.measurement);
            indexKey.push_back('\0');
            indexKey.append(tagKey);
            indexKey.push_back('\0');
            indexKey.append(tagValue);
            indexKey.push_back('\0');
            indexKey.append(encodedId);
            batch.Put(indexKey, encodedId);
    ...
    // Phase 4a: Pre-load all cache misses BEFORE the mutation loop to avoid
    // co_await inside the loop (which would allow concurrent coroutines to
    // interleave and create inconsistent batch state).
    // All LevelDB reads are batched into a single seastar::async block to reduce
    // N co_awaits (N = number of unique measurements + tag keys with cache misses)
    // down to at most 1 co_await.
    {
        std::vector<std::string> measurementsNeedingFieldLoad;
        std::vector<std::string> measurementsNeedingTagLoad;
        // tvCacheKey -> (measurement, tagKey) for tag values that need loading
        std::vector<std::pair<std::string, std::pair<std::string, std::string>>> tvLoads;

        // Use sets to deduplicate (multiple ops may share the same measurement)
        std::unordered_set<std::string> fieldLoadSeen, tagLoadSeen, tvLoadSeen;

        for (const auto& op : ops) {
            if (fieldsCache.find(op.measurement) == fieldsCache.end() && fieldLoadSeen.insert(op.measurement).second) {
                measurementsNeedingFieldLoad.push_back(op.measurement);
            }
            if (tagsCache.find(op.measurement) == tagsCache.end() && tagLoadSeen.insert(op.measurement).second) {
                measurementsNeedingTagLoad.push_back(op.measurement);
            }
            for (const auto& [tagKey, tagValue] : op.tags) {
                std::string tvCacheKey;
                tvCacheKey.reserve(op.measurement.size() + 1 + tagKey.size());
                tvCacheKey.append(op.measurement);
                tvCacheKey.push_back('\0');
                tvCacheKey.append(tagKey);
                if (tagValuesCache.find(tvCacheKey) == tagValuesCache.end() && tvLoadSeen.insert(tvCacheKey).second) {
                    tvLoads.emplace_back(std::move(tvCacheKey), std::make_pair(op.measurement, tagKey));
                }
            }
        }

        if (!measurementsNeedingFieldLoad.empty() || !measurementsNeedingTagLoad.empty() || !tvLoads.empty()) {
            // Single seastar::async block for all LevelDB reads
            struct BatchLoadResults {
                std::vector<std::pair<std::string, std::set<std::string>>> fields;
                std::vector<std::pair<std::string, std::set<std::string>>> tagKeys;
                std::vector<std::pair<std::string, std::set<std::string>>> tagValues;
            };

            auto loaded =
                co_await seastar::async([this, &measurementsNeedingFieldLoad, &measurementsNeedingTagLoad, &tvLoads] {
                    BatchLoadResults result;
                    leveldb::ReadOptions readOpts;

                    for (const auto& m : measurementsNeedingFieldLoad) {
                        std::string key = encodeMeasurementFieldsKey(m);
                        std::string val;
                        std::set<std::string> decoded;
                        if (db->Get(readOpts, key, &val).ok()) {
                            decoded = decodeStringSet(val);
                        }
                        result.fields.emplace_back(m, std::move(decoded));
                    }

                    for (const auto& m : measurementsNeedingTagLoad) {
                        std::string key = encodeMeasurementTagsKey(m);
                        std::string val;
                        std::set<std::string> decoded;
                        if (db->Get(readOpts, key, &val).ok()) {
                            decoded = decodeStringSet(val);
                        }
                        result.tagKeys.emplace_back(m, std::move(decoded));
                    }

                    for (const auto& [tvCacheKey, measTag] : tvLoads) {
                        const auto& [m, tk] = measTag;
                        std::string key = encodeTagValuesKey(m, tk);
                        std::string val;
                        std::set<std::string> decoded;
                        if (db->Get(readOpts, key, &val).ok()) {
                            decoded = decodeStringSet(val);
                        }
                        result.tagValues.emplace_back(tvCacheKey, std::move(decoded));
                    }

                    return result;
                });

            // Merge loaded data into caches (back on reactor thread)
            for (auto& [m, fields] : loaded.fields) {
                fieldsCache[m].insert(fields.begin(), fields.end());
            }
            for (auto& [m, tagKeys] : loaded.tagKeys) {
                tagsCache[m].insert(tagKeys.begin(), tagKeys.end());
            }
            for (auto& [tvCacheKey, values] : loaded.tagValues) {
                tagValuesCache[tvCacheKey].insert(values.begin(), values.end());
            }
    ...
    // Phase 4b: Mutation loop — no co_await, runs atomically.
    for (const auto& op : ops) {
        // Track field additions via in-memory cache
        if (fieldsCache[op.measurement].insert(op.fieldName).second) {
            newFields[op.measurement].insert(op.fieldName);
        }

        // Track tag additions via in-memory caches
        for (const auto& [tagKey, tagValue] : op.tags) {
            if (tagsCache[op.measurement].insert(tagKey).second) {
            ...
            tvCacheKey.append(tagKey);
            if (tagValuesCache[tvCacheKey].insert(tagValue).second) {
                newTagValues[std::move(tvCacheKey)].insert(tagValue);
            }
        }

        // Track field type via in-memory cache
        std::string fieldTypeCacheKey;
        fieldTypeCacheKey.reserve(op.measurement.size() + 1 + op.fieldName.size());
        fieldTypeCacheKey.append(op.measurement);
        fieldTypeCacheKey.push_back('\0');
        fieldTypeCacheKey.append(op.fieldName);
        if (knownFieldTypes.find(fieldTypeCacheKey) == knownFieldTypes.end()) {
            std::string typeStr;
            switch (op.valueType) {
    ...
    // Caches are already pre-loaded from Phase 4a, so no LevelDB loads needed here.
    ...
    for (const auto& [tvCacheKey, tagValues] : newTagValues) {
        ...
        key = encodeTagValuesKey(measurement, tagKey);
        batch.Put(key, encodeStringSet(tagValuesCache[tvCacheKey]));
        batchHasData = true;
    }
...
seastar::future<std::expected<std::vector<SeriesId128>, SeriesLimitExceeded>> LevelDBIndex::findSeries(
    const std::string& measurement, const std::map<std::string, std::string>& tagFilters, size_t maxSeries) {
    timestar::index_log.debug("findSeries called for measurement: {}, with {} tag filters, maxSeries: {}", measurement,
                              tagFilters.size(), maxSeries);

    if (tagFilters.empty()) {
        // Return all series for measurement (pass limit through for early bailout)
        timestar::index_log.debug("No tag filters, getting all series for measurement");
    ...
    for (const auto& [tagKey, tagValue] : tagFilters) {
        ...
        if (timestar::SeriesMatcher::classifyScope(tagValue) == timestar::ScopeMatchType::EXACT) {
        ...
        timestar::index_log.debug("TAG_INDEX filter {}={} returned {} series", tagKey, tagValue, tagResults.size());

        // If any tag filter returns empty, the intersection is empty — short-circuit.
        if (tagResults.empty()) {
            timestar::index_log.info("findSeries returning 0 results for measurement {} (empty tag filter {}={})",
    ...
    if (maxSeries > 0 && results.size() > maxSeries) {
        timestar::index_log.info("findSeries: {} results exceed limit of {} for measurement {}", results.size(),
                                 maxSeries, measurement);
        co_return std::unexpected(SeriesLimitExceeded{results.size(), maxSeries});
    }

    timestar::index_log.info("findSeries returning {} results for measurement {}", results.size(), measurement);
    co_return results;
}
...
seastar::future<std::expected<std::vector<SeriesId128>, SeriesLimitExceeded>> LevelDBIndex::getAllSeriesForMeasurement(
    ...
    if (cacheIt != measurementSeriesCache.end()) {
        timestar::index_log.debug("getAllSeriesForMeasurement cache hit for '{}': {} series", measurement,
                                  cacheIt->second.size());
    ...
    auto seriesIds = co_await seastar::async([this, &measurement, maxSeries]() {
        ...
        // Use fill_cache=false to avoid polluting the block cache with scan data
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
        if (!it) {
            timestar::index_log.error("Failed to create LevelDB iterator");
            return result;
        }
        ...
        if (!it->status().ok()) {
            timestar::index_log.error("MEASUREMENT_SERIES iterator error: {}", it->status().ToString());
        }
    ...
    if (maxSeries > 0 && seriesIds.size() > maxSeries) {
        timestar::index_log.info("getAllSeriesForMeasurement: {} series exceed limit of {} for '{}'", seriesIds.size(),
                                 maxSeries, measurement);
        co_return std::unexpected(SeriesLimitExceeded{seriesIds.size(), maxSeries});
    }
    ...
    if (!seriesIds.empty()) {
        timestar::index_log.debug("MEASUREMENT_SERIES index found {} series for '{}'", seriesIds.size(), measurement);
        auto& cached = measurementSeriesCache[measurement];
        // Use a set for O(n log n) dedup instead of O(n²) std::find loop
        std::set<SeriesId128> existing(cached.begin(), cached.end());
        for (auto& id : seriesIds) {
    ...
    timestar::index_log.debug("MEASUREMENT_SERIES index empty for '{}', falling back to SERIES_METADATA scan",
                              measurement);
    ...
    seriesIds = co_await seastar::async([this, &measurement, maxSeries]() {
        ...
        // Use fill_cache=false for full-table fallback scan
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
        if (!it) {
        ...
        while (it->Valid()) {
            ...
            if (!valueSlice.empty()) {
                try {
                    ...
                    timestar::index_log.error("Failed to decode series metadata: {}", e.what());
                }
            }
        ...
        if (!it->status().ok()) {
            timestar::index_log.error("SERIES_METADATA fallback iterator error: {}", it->status().ToString());
        }
    ...
    if (maxSeries > 0 && seriesIds.size() > maxSeries) {
        timestar::index_log.info("getAllSeriesForMeasurement fallback: {} series exceed limit of {} for '{}'",
                                 seriesIds.size(), maxSeries, measurement);
        // Don't cache partial results from limit-exceeded scans
        co_return std::unexpected(SeriesLimitExceeded{seriesIds.size(), maxSeries});
    }

    timestar::index_log.info("Fallback scan found {} series for measurement '{}'", seriesIds.size(), measurement);
...
seastar::future<> LevelDBIndex::rebuildMeasurementSeriesIndex() {
    ...
    timestar::index_log.info("Rebuilding MEASUREMENT_SERIES index from SERIES_METADATA entries...");
    ...
    auto count = co_await seastar::async([this]() {
        ...
        // Use fill_cache=false for rebuild scan to avoid polluting the block cache
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
        if (!it) {
            timestar::index_log.error("Failed to create LevelDB iterator for rebuild");
            return cnt;
        }
            ...
            if (keySlice.size() >= 17) {
                ...
                if (!valueSlice.empty()) {
                    try {
                        ...
                        timestar::index_log.error("Failed to decode metadata during rebuild: {}", e.what());
                    }
                }
            }
        ...
        if (!it->status().ok()) {
            timestar::index_log.error("Iterator error during rebuild: {}", it->status().ToString());
        }
    ...
    timestar::index_log.info("MEASUREMENT_SERIES index rebuilt: {} entries written", count);
    co_return;
}
...
seastar::future<size_t> LevelDBIndex::getSeriesCount() {
    ...
    auto count = co_await seastar::async([this] {
        size_t cnt = 0;
        // Use fill_cache=false for count scan to avoid polluting the block cache
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
...
seastar::future<> LevelDBIndex::compact() {
    ...
    co_await seastar::async([this] { db->CompactRange(nullptr, nullptr); });
    co_return;
}
...
seastar::future<std::vector<SeriesId128>> LevelDBIndex::findSeriesByTag(const std::string& measurement,
    ...
    auto seriesIds = co_await seastar::async([this, &measurement, &tagKey, &tagValue, maxSeries] {
        ...
        std::string keyPrefix;
        keyPrefix.reserve(1 + measurement.size() + 1 + tagKey.size() + 1 + tagValue.size() + 1);
        ...
        // Scan all keys with this prefix; fill_cache=false to avoid polluting block cache
...
seastar::future<std::vector<SeriesId128>> LevelDBIndex::findSeriesByTagPattern(const std::string& measurement,
                                                                               const std::string& tagKey,
    ...
    std::string literalPrefix = timestar::SeriesMatcher::extractLiteralPrefix(scopeValue);
    ...
    auto seriesIds = co_await seastar::async([this, &keyPrefix, &seekKey, &literalPrefix, &scopeValue, maxSeries] {
        std::vector<SeriesId128> result;

        // Pre-compile regex once if this is a regex pattern, to avoid per-entry compilation
        auto matchType = timestar::SeriesMatcher::classifyScope(scopeValue);
        std::optional<std::regex> compiledRegex;
        std::string regexPattern;

        if (matchType == timestar::ScopeMatchType::REGEX) {
        ...
        // fill_cache=false to avoid polluting the block cache with scan data
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
        leveldb::Slice prefixSlice(keyPrefix);
        ...
        for (it->Seek(seekKey); it->Valid(); it->Next()) {
            ...
            if (!literalPrefix.empty() &&
                (tagValue.size() < literalPrefix.size() || tagValue.substr(0, literalPrefix.size()) != literalPrefix)) {
                break;
            }
            ...
            if (compiledRegex) {
                ...
                matched = timestar::SeriesMatcher::matchesTag(tvStr, scopeValue);
            }
        ...
        if (!it->status().ok()) {
            throw std::runtime_error("Failed to iterate tag index for pattern: " + it->status().ToString());
        }
...
seastar::future<std::map<std::string, std::vector<SeriesId128>>> LevelDBIndex::getSeriesGroupedByTag(
    ...
    auto grouped = co_await seastar::async([this, &measurement, &tagKey] {
        ...
        std::string keyPrefix;
        keyPrefix.reserve(1 + measurement.size() + 1 + tagKey.size() + 1);
...
seastar::future<> LevelDBIndex::updateFieldStats(const SeriesId128& seriesId, const std::string& field,
    ...
    // Offload blocking LevelDB I/O to Seastar thread pool
    auto status =
        co_await seastar::async([this, &key, &value] { return db->Put(leveldb::WriteOptions(), key, value); });
    if (!status.ok()) {
...
seastar::future<std::optional<IndexFieldStats>> LevelDBIndex::getFieldStats(const SeriesId128& seriesId,
    ...
    if (status.ok()) {
        ...
        auto findDelim = [&](size_t from) -> size_t {
            size_t p = value.find('\0', from);
            if (p == std::string::npos) {
                throw std::runtime_error("Corrupt FieldStats: missing null delimiter");
            }
            return p;
        };

        try {
            size_t nextPos = findDelim(pos);
            stats.dataType = value.substr(pos, nextPos - pos);
            pos = nextPos + 1;

            nextPos = findDelim(pos);
...
template seastar::future<SeriesId128> LevelDBIndex::indexInsert<double>(const TimeStarInsert<double>& insert);
template seastar::future<SeriesId128> LevelDBIndex::indexInsert<bool>(const TimeStarInsert<bool>& insert);
template seastar::future<SeriesId128> LevelDBIndex::indexInsert<std::string>(const TimeStarInsert<std::string>& insert);
template seastar::future<SeriesId128> LevelDBIndex::indexInsert<int64_t>(const TimeStarInsert<int64_t>& insert);
...
seastar::future<std::vector<RetentionPolicy>> LevelDBIndex::getAllRetentionPolicies() {
    ...
    auto policies = co_await seastar::async([this] {
        ...
        // Use fill_cache=false for scan to avoid polluting the block cache
        leveldb::ReadOptions scanOpts;
        scanOpts.fill_cache = false;
        std::unique_ptr<leveldb::Iterator> it(db->NewIterator(scanOpts));
        it->Seek(prefix);
...
seastar::future<bool> LevelDBIndex::deleteRetentionPolicy(const std::string& measurement) {
    ...
    auto deleteStatus = co_await seastar::async([this, &key] { return db->Delete(leveldb::WriteOptions(), key); });
    if (!deleteStatus.ok()) {
