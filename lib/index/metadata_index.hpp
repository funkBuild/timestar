...
public:
    ...
    seastar::future<std::vector<uint64_t>> findSeriesByTag(const std::string& measurement, const std::string& tagKey,
                                                           const std::string& tagValue);
    ...
    seastar::future<> updateFieldStats(const std::string& measurement, const std::string& field, uint64_t seriesId,
                                       const FieldStats& stats);

    // Get series IDs grouped by tag value
    seastar::future<std::map<std::string, std::vector<uint64_t>>> getSeriesGroupedByTag(const std::string& measurement,
