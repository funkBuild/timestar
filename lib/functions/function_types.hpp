...
public:
    ...
    TimeSeriesView(const std::vector<uint64_t>* ts, const std::vector<T>* vals, size_t start = 0, size_t cnt = 0)
        : timestamps(ts),
          values(vals),
          startIndex(start),
          count((!ts || !vals)
    ...
    struct iterator {
        ...
        std::pair<uint64_t, T> operator*() const { return {view->timestampAt(index), view->valueAt(index)}; }
...
struct FunctionResult {
    ...
    TimeSeriesView<T> asView() const { return TimeSeriesView<T>(&timestamps, &values); }
};
...
public:
    void setParameter(const std::string& name, const ParameterValue& value) { parameters_[name] = value; }
...
public:
    explicit FunctionException(const std::string& message) : std::runtime_error("Function error: " + message) {}
};
...
public:
    ...
    const FunctionMetadata& getMetadata() const override { return registryMetadata_; }
...
public:
    virtual seastar::future<FunctionResult<double>> execute(const DoubleSeriesView& input,
...
public:
    ...
    std::unique_ptr<IFunction> clone() const override {
        ...
        return std::make_unique<RegistryUnaryFunctionWrapper>(std::unique_ptr<IUnaryFunction>(castedFunction),
                                                              registryMetadata_);
    }
...
public:
    ...
    bool isSimdAligned() const { return !data_.empty() && (reinterpret_cast<uintptr_t>(data_.data()) % 32) == 0; }
    ...
    double average() const { return empty() ? std::numeric_limits<double>::quiet_NaN() : sum() / data_.size(); }
...
public:
    AlignedVectorizedSeries(const std::vector<uint64_t>& timestamps, const std::vector<double>& values)
        : timestamps_(timestamps), values_(values) {
        if (timestamps_.size() != values_.size()) {
            throw std::invalid_argument("AlignedVectorizedSeries: timestamps and values must have the same size (" +
...
public:
    static void validateMinDataPoints(size_t actual, size_t required, const std::string& functionName) {
        if (actual < required) {
            throw InsufficientDataException(functionName + " requires at least " + std::to_string(required) +
    ...
    static std::pair<std::vector<uint64_t>, std::pair<std::vector<size_t>, std::vector<size_t>>> alignTimeSeries(
        const std::vector<uint64_t>& timestamps1, const std::vector<uint64_t>& timestamps2) {
