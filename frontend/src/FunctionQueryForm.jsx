...
function buildFormula(functionType, params) {
  if (functionType === 'anomaly') {
    return `anomalies(a, '${params.algorithm}', ${params.bounds}, '${params.seasonality}')`;
  }
  if (params.algorithm === 'seasonal') {
    return `forecast(a, 'seasonal', ${params.deviations}, seasonality='${params.seasonality}')`;
  }
  return `forecast(a, '${params.algorithm}', ${params.deviations})`;
}

export default function FunctionQueryForm({ measurements, onSubmit, loading }) {
  const qb = useQueryBuilder();
  const [functionType, setFunctionType] = useState('anomaly');

  // Anomaly params
  const [anomalyAlgo, setAnomalyAlgo] = useState('basic');
  const [bounds, setBounds] = useState(2);
  const [anomalySeasonality, setAnomalySeasonality] = useState('none');

  // Forecast params
  const [forecastAlgo, setForecastAlgo] = useState('linear');
  const [deviations, setDeviations] = useState(2);
  const [forecastSeasonality, setForecastSeasonality] = useState('daily');
  ...
  const params = functionType === 'anomaly'
    ? { algorithm: anomalyAlgo, bounds, seasonality: anomalySeasonality }
    : { algorithm: forecastAlgo, deviations, seasonality: forecastSeasonality };
  const formula = qb.queryString ? buildFormula(functionType, params) : '';
  const canSubmit = !!qb.selectedMeasurement && !!qb.queryString;

  const handleSubmit = useCallback(() => {
    if (!canSubmit) return;
    let aggregationInterval = null;
    if (functionType === 'forecast' && intervalValue && Number(intervalValue) > 0) {
      aggregationInterval = `${intervalValue}${intervalUnit}`;
    }
    onSubmit({
      queries: { a: qb.queryString },
      formula: buildFormula(functionType, params),
      startTime,
      endTime,
      aggregationInterval,
      functionType,
    });
  }, [canSubmit, qb.queryString, functionType, params, startTime, endTime, intervalValue, intervalUnit, onSubmit]);
  ...
  return (
    <div className="query-form">
      <div className="form-section">
        <div className="form-section-label">Function</div>
      ...
      {functionType === 'anomaly' && (
        <>
          <div className="form-section">
            <div className="form-section-label">Algorithm</div>
            <select value={anomalyAlgo} onChange={(e) => setAnomalyAlgo(e.target.value)}>
          ...
          <div className="form-section">
            ...
            <input type="number" min="1" max="4" step="0.5" value={bounds} onChange={(e) => setBounds(Number(e.target.value))} />
          </div>
          <div className="form-section">
            <div className="form-section-label">Seasonality</div>
            <select value={anomalySeasonality} onChange={(e) => setAnomalySeasonality(e.target.value)}>
      ...
      {functionType === 'forecast' && (
        <>
          ...
          <div className="form-section">
            ...
            <input type="number" min="1" max="4" step="0.5" value={deviations} onChange={(e) => setDeviations(Number(e.target.value))} />
          </div>
          {forecastAlgo === 'seasonal' && (
            <div className="form-section">
              <div className="form-section-label">Seasonality</div>
              <select value={forecastSeasonality} onChange={(e) => setForecastSeasonality(e.target.value)}>
                <option value="none">none</option>
                <option value="hourly">hourly</option>
                <option value="daily">daily</option>
                <option value="weekly">weekly</option>
          ...
          <div className="form-section">
            <div className="form-section-label">
              Interval {forecastAlgo === 'seasonal' && <span style={{ color: 'var(--danger)', marginLeft: 4 }}>(required)</span>}
            </div>
            <div className="interval-row">
              <input type="number" min="0" step="any" placeholder="e.g. 5" value={intervalValue} onChange={(e) => setIntervalValue(e.target.value)} />
      ...
      <div className="submit-section">
        <button className="submit-btn" disabled={!canSubmit || loading} onClick={handleSubmit}>
          {loading ? 'Running...' : functionType === 'anomaly' ? 'Detect Anomalies' : 'Run Forecast'}
        </button>
        {formula && <div className="query-preview">{formula}</div>}
      </div>
    </div>
  );
}
