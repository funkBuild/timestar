...
**State as of 2026-07-27 (`1080130`, tip of `cluster-design`):** closed so far — D-1,
D-2, D-3, D-4 (two of three halves; third re-filed as D-19), D-5, D-6, D-7, D-11, D-12,
D-13, D-15, D-16, D-17, D-33, D-34, D-35; D-14 materially improved. D-10 is BUILT but
...
into deleted segment files. **The standout open correctness item is D-36** (acked writes
not readable after a restart that follows a heavy write campaign; loss vs apply-stall
undetermined) — it predates the snapshot work and nothing in this campaign has touched
it. Suites at last green: unit 4227, socket 37.
...
| ~~D-34~~ | **CLOSED** (`1080130`). `JournalGc`/`JournalRetention` have a production caller: `ReplicatedVShardHost::reclaimJournalSegments`, riding the D-6 snapshot sweep's timer at its own 60 s interval (compaction is the only thing that moves a floor, so nothing is collectable that a snapshot did not cause -- but a collect reads whole sealed segments, so it must not run every 5 s). One pass at a time; a failure is logged with the sweep's and retried, because the unlink is the LAST step of each segment. **THE FLOOR IS NOT THE SNAPSHOT RECORD, and that is the whole of the difficulty.** The obvious rule -- release everything below the Snapshot record's `vshard_seq` -- is wrong twice, because that record is not the newest record of the things it does not describe. (i) The LOG ENTRIES ABOVE THE BOUNDARY were appended BEFORE it (they are already in the log when compaction runs), so their seqs are LOWER; releasing to the snapshot deletes exactly the unflushed suffix D-6 retains on purpose. (ii) The HARD STATE is persisted only when term/vote CHANGE, so a group with stable leadership has ONE HardState record, from startup, below everything -- deleting it loses this replica's vote, a Raft correctness violation rather than a saving. `JournalRaftPersistence::releasedSeq()` is therefore `min(newest HardState seq, newest Snapshot seq, seq of the OLDEST still-live entry) - 1`, and **0 until a snapshot exists at all**. The live-entry term mirrors what `recoverRaftState` rebuilds (an append at index I supersedes every record at or above I; a snapshot at B retires everything at or below B) at 16 bytes per entry, against a log entry that is a whole WriteBatch. **The hard-state pin is why compaction now re-persists the hard state:** without it an idle-term group is frozen at its first record forever and reclaims NOTHING, which is the common case; one ~40-byte record moves the pin, and replaying it is idempotent. **A RESTART MUST NOT RESET THE FLOOR UPWARD** -- a fresh persistence object knows none of the on-disk seqs, so its "oldest live entry" would be the first entry appended AFTER the restart and the floor would jump over the recovered suffix; `recoverRaftState` now reports a retention seed and `addVShard` feeds it in before anything is appended (the recovered floor also primes `retention_`, so a restart over a compacted journal collects on its FIRST pass). **Both layouts, one rule.** Per-VShard journals (the default): a segment holds one group's records, its own floor decides everything, and copy-forward could gain at most the one partially-released 1 MiB segment -- so it runs DELETE-ONLY, never touching the writer, which is what makes it safe alongside the group's own appends; only VShards whose floor MOVED are visited, so an idle node does not walk ~1365 directories a minute. Shared journal (D-10): a segment holds every group on the reactor, one laggard pins 64 MiB for everyone, so copy-forward is on -- inside a new `SharedShardJournal::runExclusive` held for the WHOLE copy, not per call, because a round that barriered between two relocated records would report them durable while the source segment is about to be unlinked; bounded by a live-record budget (a mostly-live segment is pinned instead), which also bounds the exclusive section. **Crash during GC is safe by ORDERING:** copy forward, BARRIER, unlink, sync the directory. Every window leaves at least one complete generation, and the overlap window leaves a byte-identical DUPLICATE -- `recoverRaftState` replays in vshard_seq order so a repeat re-applies identically, and `JournalReplay::finalize` drops exact duplicates while failing closed on a conflicting one. A partially-deleted sequence is fine for the same reason each deletion removes only records at or below a released watermark: what survives is a gap-free suffix per VShard. **Evidence:** 13 tests, each run against a deliberately broken variant -- the naive below-the-snapshot floor fails 3 of the 5 floor tests, a seed omitting the surviving entries fails the restart test, removing the pin branch fails 4 GC tests, removing finalize's dedupe fails the crash-window test, dropping runExclusive's lock fails the exclusion test, and publishing `nextSeq - 1` as the floor deletes the segment holding the snapshot record so the end-to-end host test fails on *"the boundary must survive reclamation"*. The end-to-end test drives the real wiring (write 24 entries -> flush -> snapshot -> publish -> collect) and asserts BOTH directions: 0 segments deleted before any snapshot, > 0 after, a second pass at the same floor is a no-op, and a restart over the reclaimed journal still reads all 24 acknowledged points with its boundary intact. `setJournalSegmentBytes` is the test seam and the honest operator knob -- reclamation only acts on SEALED segments, and proving one at the production 1 MiB target means pushing a megabyte of entries through a quorum. Suites at unit **4227/4227**, socket **37/37**. Residuals D-39 (shared-layout reclamation is rate-limited by the D-6 sweep, not by this), D-40 (a VShard that MOVES AWAY pins its records in a shared segment forever). | `lib/storage/journal_gc.{hpp,cpp}`, `journal_sink.{hpp,cpp}` (`runExclusive`); `raft_journal_persistence.{hpp,cpp}` (`releasedSeq`/`seedRetention`), `raft_group.cpp` (hard-state re-persist); `replicated_vshard_host.{hpp,cpp}` (`reclaimJournalSegments`) | snapshot owner |
| ~~D-35~~ | **CLOSED** (`539cbf4`). `EngineLocalStore::hasUnconvertedStores` now asks a PER-VSHARD question. The refusal itself is unchanged and still right (D-6 paid for it in blood: conversions run six at a time and complete OUT OF ORDER, so "the highest revision in TSM" can sit above an unconverted store's revisions and truncating there discarded 7 of 200 acknowledged points across a kill -9) -- what changed is the SCOPE of the question. `hasPendingConversions` is per SHARD, so all ~1365 groups on a core got the same answer, and a shard under sustained ingest is essentially never at zero pending conversions: on a busy node NOTHING compacted, and D-6's mechanism idled exactly when the logs were growing fastest. **The safety argument never involved the other VShards' stores** -- if no unconverted rolled store holds VShard V, every rolled point of V is in TSM and V's only unflushed remainder is in the ACTIVE store, a contiguous SUFFIX of V's revisions, which is what makes the max flushed revision a boundary at all. **Mechanism:** a 4096-bit presence bitmap per `MemoryStore` (512 bytes, never grows, never allocates), one bit set per insert from `hash & 4095` -- the same pure function the router uses -- hung off `insertMemory`, the single lowest-level insertion path, so WAL replay (`initFromWAL` -> `WALReader::readAll` -> `insertMemory`) populates it identically to a live write. A recovered store that under-reported its VShards would be precisely the silent hole this closes. **Two store-list semantics are load-bearing** and are stated once, in a pure function: `memoryStores[0]` is the ACTIVE store and never counts (`rolloverMemoryStore` inserts the fresh store at the FRONT), and conversion erases a store only AFTER `addTSMFile` -- the same visibility ordering queries rely on -- so the rolled set is exactly `[1, size)`. **Not the sketched route, deliberately:** the register proposed a contiguous-completion watermark over `TSMFileManager::reserveSequenceId`, but a TSM file's seq is re-assigned by COMPACTION, so "files with dataSeq <= W" is not a stable stand-in for "written by a converted store"; the bitmap answers the question directly and exactly rather than through an ordering proxy. Still conservative in the same direction: a VShard whose OWN data is converting continuously keeps its log. **Inherited hole, called out rather than introduced:** a store whose conversion fails twice is erased from the vector with its data still only in its WAL file, and BOTH predicates then read "nothing pending" for it -- pre-existing, and it belongs to the conversion-failure path. 3 tests; negative controls: dropping the `noteVShard` call fails the bitmap test and the predicate test, and starting the scan at index 0 (counting the active store) fails the predicate test and the active-store-alone test. Suites at unit **4227/4227**, socket **37/37**. | `memory_store.{hpp,cpp}` (`noteVShard`), `wal_file_manager.hpp` (`pendingConversionForVShard`), `engine.hpp`, `engine_local_store.{hpp,cpp}` | snapshot owner + storage owner |
| D-36 | **ACKED WRITES ARE NOT READABLE AFTER A RESTART THAT FOLLOWS A HEAVY WRITE CAMPAIGN (loss vs apply-stall UNDETERMINED). Pre-existing, not snapshot-related, and it violates the ack contract either way.** Found by `snapshot_durability_gate.sh` while establishing that D-6 does not cost durability. Same session, same box, 200 acked single-point probes, whole cluster kill -9'd and restarted: **with a 150x10k load phase, readback lands anywhere in 175-199 of 200 with compaction OFF and 185-200 with it ON** (four arms across two runs: 193/185, then 180/200) -- and with a LIGHT load phase, **200/200/200 with compaction on and 14 snapshots taken**. So restart over compacted journals is sound, the shortfall appears only after a heavy campaign, it is not correlated with snapshotting, and its magnitude swings run to run by more than any effect being looked for. **THE TITLE SAYS "not readable" AND NOT "lost" DELIBERATELY, per review:** `drainReady` propagates an `apply()` throw BEFORE `node_.advance(rd)`, so the suspected mechanism -- restart replay of ~1.5M points tripping `rejectIfIngestBacklogged` inside apply, which `EngineDataStateMachine`'s own header already warns about ("do not read this as never fires") -- produces a temporary apply STALL, not permanent loss. The gate reads back exactly ONCE and never re-reads, so its evidence cannot separate the two, and the ingest-backlog log-line counts do not settle it either (12 vs 11 on the run where the arms differed by 8 points). A THIRD candidate ranks equally and is not yet excluded: **journal tail recovery dropping a contiguous suffix after kill -9** -- the probes ARE the tail, which is exactly the shape that would produce. **Wants, cheapest first:** (1) re-read after a bounded delay, which separates stall from loss in one line; (2) count committed-but-unapplied entries directly after recovery rather than inferring from a query; (3) an apply-path insert that bypasses admission control (admission belongs at PROPOSE time, which is where the design already puts it); (4) a journal-tail test that acks N writes, kill -9s, and checks the recovered record count against N. The gate reports this as an advisory and fences D-6 with the light-load absolute bar instead, precisely so this is not mistaken for a D-6 regression. | `engine_data_state_machine.cpp` (`apply`), `engine.cpp` (`rejectIfIngestBacklogged` on the insert path), `journal_replay.cpp` / `journal_writer.cpp` (tail recovery); gate `test/cluster_gates/snapshot_durability_gate.sh` | storage owner + snapshot owner. Pre-existing |
| D-37 | **Snapshot flow control is per-peer PER GROUP, so there is no NODE-level cap on concurrent transfers.** The one-unacked-chunk pacing is held in each `RaftNode`'s `snapTransfers_`, and a node hosts ~1365 groups per shard. A node returning after an outage long enough for its peers to have compacted can therefore be the target of up to that many SIMULTANEOUS transfers, each willing to put a 4 MiB chunk on the wire: the per-message bound holds, the per-group pacing holds, and the aggregate is bounded by nothing but the peer's inbound admission semaphore -- which queues rather than drops, so it degrades into latency and stalled transfers rather than corruption. Consistent with what the restart-catchup gate already shows: 10-15 stall-driven resends among 18-19 chunks on a 3-node cluster where only a handful of groups needed a snapshot at all. Filed rather than fixed because it is an N->N+1 scale question (a fresh node joining a large cluster is the real case, not the 3-node gate), and the fix is a shard- or node-level concurrency budget that transfers acquire before their first chunk. | `raft_node.hpp` (`snapTransfers_`), `raft_rpc_transport.cpp` (`kMaxInboundRaftMemory`); movement/M5 join path | snapshot owner + movement/M5 |
...
| D-39 | **Shared-journal segment reclamation is rate-limited by the SNAPSHOT sweep, not by the GC.** Filed while closing D-34 and it only bites with `TIMESTAR_CLUSTER_SHARED_JOURNAL=1`. A segment there holds every group on the reactor, so it is reclaimable only once EVERY group's floor has passed every record in it (or the stragglers are copied forward, which D-34 caps at `maxCopyForwardRecords` = 512 so a mostly-live segment is pinned rather than rewritten). But a floor only moves when its group COMPACTS, and D-6's sweep takes at most **one snapshot per shard per 5 s** with a 60 s per-group minimum, over ~1365 groups per shard: a full cycle is **~1.9 hours**, so at any moment the overwhelming majority of groups sit at floor 0 and every 64 MiB segment is mostly live. Reclamation in that layout is therefore real but slow, and its steady-state disk footprint is set by the sweep rate rather than by anything in the GC. The per-VShard default has no such coupling — a segment holds one group, so its own compaction releases it. **Wants** either a higher `kMaxConcurrentSnapshots`/shorter sweep when the shared journal is on (the cost is reading and encoding N VShards' TSM files on one reactor, which is exactly what the rate limit exists to prevent), or a cheap floor-advance that does not require a full snapshot — e.g. publishing a floor from the already-known flush watermark without producing a payload. **Evidence to gather first:** `journalSegmentsPinned()` vs `journalSegmentsDeleted()` on a shared-journal gate run; nothing measures it today. | `replicated_vshard_host.hpp` (`kSnapshotSweepInterval`, `kMaxConcurrentSnapshots`, `kMinSnapshotInterval`); `journal_gc.hpp` (`maxCopyForwardRecords`) | snapshot owner |
| D-40 | **A VShard that MOVES AWAY pins its shared-journal records forever.** Also filed while closing D-34, also shared-layout only. The reclaim floor is published per VShard from the group's own `JournalRaftPersistence`, so a group that is no longer hosted has nobody to advance it: `retention_` keeps its last value (the map is monotonic and nothing removes entries), and every segment holding a record above that value is pinned for the life of the process. In the per-VShard layout the same event is harmless because the VShard's whole directory is deleted outright on move, taking its segments with it — there is no shared file to pin. **The fix is a retirement step**, not a floor change: when a VShard leaves this node its records are dead by definition (the replica is gone), so movement must publish `UINT64_MAX` — or an explicit `retire(vshard)` — into `JournalRetention` at the same point it tears the group down. That is a movement/M5 wiring change and is deliberately NOT done blind here: publishing "everything released" for a group that is merely momentarily absent (an addVShard that has not run yet, a restart mid-move) would delete a live replica's log, so it must hang off the movement decision and not off `vshards_.count() == 0`. Until then the shared journal leaks segments across moves; the default layout does not. | `replicated_vshard_host.cpp` (`publishReclaimFloors`, `retention_`); movement/M5 VShard teardown | movement/M5 + snapshot owner |
