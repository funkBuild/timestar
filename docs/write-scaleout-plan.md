...
5d. **DONE (debt D-9): CheckQuorum is enabled, behind a leader-transfer bypass AND a
    hibernation fix.** `82591a9` (the bypass: a transfer vote under its own message-type
    byte, honoured at the inLease check), `5bdaa03` (the flag), `277e110` (the second
    blocker, which this entry and ADR 0005 both missed: hibernation stretches the LEASE
    tenfold, so a dead leader's groups could not re-elect for 25-50 s — measured as
    153/400 failed batches and a 43 s recovery on `node_kill_round.sh` against a
    same-session 49/400 and 8 s control; a vote the lease DROPS now un-hibernates the
    voter, giving 62-69/400 and 12 s). Residual band cost is debt D-29; the ADR's
    mechanism (c), cluster-wide gated activation, is debt D-30. Original entry follows,
    unedited, because it is the record of why the revert was right.
...
| ~~D-8~~ | **CLOSED** (`c2bccf8`, message fix `5412d4f`). The bound now covers BOTH doors of the RF>1 write path, as **two separate per-shard budgets** (`cluster::AdmissionClass`): originated (charged on the request shard, as before) and **peer ingress** (charged on the SERVING shard — the one the peer's connection landed on, which is where the decoded batch and the fan-out frame live). That serving-shard charge is the "different accounting shape" this row asked for, and it falls out for free: the ingress helpers are coroutines, so the guard's frame begins and ends on the same reactor. **Separate rather than shared, deliberately** — one counter makes the coordinator and replica roles compete for a number they cannot share, so a burst of replication 503s a client's own writes on a node coordinating almost nothing (and the reverse: a busy coordinator refusing replication it is the LEADER for, which its peers can only retry back at it), with the loser decided by arrival order. The cost is that the node-wide ceiling is now the SUM, which is why the default is documented per class and there is a second env var (`TIMESTAR_CLUSTER_INGRESS_INFLIGHT_BYTES`). **No double charge:** a coordinator's own slices never use the ingress door — `ReplicatedBatchWriteRouter`'s local sink is the `ReplicatedVShardHost` (`replicated_data_plane.hpp` constructs it with `host_`), so a locally-led group is proposed straight into the host and only a REMOTE leader is RPCed; the guard also carries its class into the release, so a charge cannot be released against the other counter. **What the coordinator sees** is the part that had to be got right: on the HINTED verb (the production one) the rejection is REPORTED, not thrown — every asked-about slice comes back uncommitted with kind `Overloaded` and NO leader hint. Overloaded is retryable and is NOT election-shaped, so it cannot ride D-14's 6 s window (holding a batch and its own charge for 6 s against a peer that needs something to DRAIN makes the overload worse), and reporting rather than throwing stops the coordinator reading a FULL node as an UNREACHABLE one and waking the Raft groups behind it. The legacy bool verb has no field for it, so it throws and arrives as the retryable `Transport` class — same policy, less precision. **Still uncovered, honestly:** RF=1 (`NodeWriteRouter`) has no bound at all, and **Raft's own memory** (journal append, unstable log, apply queue) is downstream of admission — a batch is released when its propose returns, while the entry it created lives on in every replica's log. Pinned by `WriteAdmissionTest.TheOriginatedAndIngressBudgetsAreIndependent`, `ShardRaftPlaneTest.PeerIngressIsChargedAndRejectsWithoutWedgingOrDoubleReleasing` (socket; the real fan-out helper, with the same call succeeding past admission once the budget is free — which is what proves admission fired) and `ReplicatedBatchWriteRouterTest.{OverloadedPeerFailsRetryablyOnTheBaseDeadline,OverloadedPeerThatDrainsIsAbsorbed}`. **Gate: `backpressure_gate.sh` PASSED** on the final binary — 16 x 503 all carrying `Retry-After`, 0 x 500, 209 server-side rejections logged at a 1 MB budget, then 200/200 clean at the default. It also EARNED its keep here: composing the message from the class name first produced "shard write write buffer full", and the gate greps the log for `shard write buffer full`, so it counted 0 rejections while clients were being correctly 503'd — an operator alert on that string would have gone quiet the same way. The originated wording is now byte-identical to what shipped before the second budget existed. | `shard_raft_plane.hpp` (`proposeSlicesToOwningShards{,Hinted}`), `write_admission.hpp`; scope note in §3d-scope | write-path owner |
| ~~D-9~~ | **CLOSED — CheckQuorum is ON** (`82591a9` the bypass, `5bdaa03` the flag, `277e110` a second blocker neither this row nor ADR 0005 predicted). (1) **The bypass**, per the ADR: `RequestVote::campaignTransfer`, set at exactly ONE site (`becomeCandidate(transfer=true)` from the TimeoutNow arm) and carried as a PARAMETER, not a member, so it cannot leak into a later self-timeout campaign; honoured at the `inLease` guard. Vote safety is untouched and that is the whole argument for accepting an unverifiable hint — it stands a LEASE down and nothing else, so §5.4.1's log check, one-vote-per-term and term ordering all still apply, and the most a lying peer buys is the configuration that shipped for months. It is self-limiting too: only the leader sends TimeoutNow, so a node cannot elect ITSELF with the flag. (2) **The wire is mechanism (b), a new type byte** (`kRequestVoteTransfer = 8`, same body): the envelope has no version field, so a byte added INSIDE `kRequestVote` would make an old decoder mis-frame every following field and produce a valid-LOOKING vote with a garbage term. An unknown type hits `decodeEnvelope`'s `default:` instead, so an old voter **drops the envelope whole** and that transfer degrades to the pre-bypass path (abandon after one election timeout, then a normal election) — slow, never a misparse. Asserted, not assumed, by `RaftCodecTest.AnOldDecoderDropsTheTransferVoteAndStillReadsAnOrdinaryOne`, which models the pre-flag switch exactly. (3) **THE PART THE ADR MISSED: hibernation stretches the lease.** An idle follower ticks 1-in-10, which stretches every tick-driven clock in the group tenfold — including the disruption guard, so 2.5-5 s of lease becomes 25-50 s. A vote arriving inside it is dropped SILENTLY (no reply, no term bump), so a group whose leader has DIED cannot be voted into a new one until the VOTER's stretched lease expires, while the candidate (woken by the write path, which only wakes groups on the node whose own writes failed) campaigns and is refused every time. Found by the gate, not by reasoning: same-session A/B on `node_kill_round.sh` differing only in the flag gave **49/400 failed batches / 8 s recovery / 3.25-3.30 M pts/s** off versus **153/400 / 43 s / 0.55 M** on, every failure `(last: transport)` — with the election stalled the coordinator had no leader and fell back to the placement primary, which was the dead node. Fixed by waking a group whose step DROPPED a vote under the lease (`RaftNode::leaseDroppedVotes` + `RaftGroupRegistry::deliver`). **The trigger is the DROP, not the vote** — waking on any RequestVote also fires for every ordinary election and every leadership transfer, and the balancer transfers thousands of groups every few seconds, so that version kept most of the map awake and failed `HibernationSkipsIdleFollowersButStillReplicates` outright. Re-measured with the fix: 62/400 and 69/400, 12 s recovery. **Gates on the final binary:** rolling_rebalance **PASS** (2216 transfers under sustained writes, 600/600 OK, 0 errors, leadership settles at 0 moved, 4.90 M pts/s) — the discriminating one, and the direct answer to 1f2e752; deposed_primary **PASS** (5 nodes RF=3, 300/300 accepted, 0 5xx, all five nodes read exactly 300 — the 274-285/300 the ADR predicted as a CheckQuorum artifact does not appear); node_kill_round **PASS**; fault_injection **PASS** (144 reset rounds / 383 connections, 2000/2000 + 200/200, 0 errors, 94 % of baseline) so D6 does not regress; canonical bench **5.02 M pts/s, 100/100, 0 errors, 0 stalls**, within 0.7 % of the 5.06 M recorded at 1f2e752 — CheckQuorum's per-tick work costs nothing on this topology, as Phase 5 said. Residuals filed: **D-29** (the band is still ~25-40 % worse than with CheckQuorum off) and **D-30** (the ADR's mechanism (c), gated activation, is NOT implemented, so a mixed-version rolling window has slow transfers). | `raft_messages.hpp`, `raft_codec.cpp`, `raft_node.{hpp,cpp}`, `raft_group_registry.{hpp,cpp}`, `cluster_data_plane.cpp`; `docs/adr/0005-checkquorum-transfer-bypass.md` | consensus owner |
...
| D-29 | **CheckQuorum still costs ~25-40 % on the one-node-down write band, and the mechanism is understood.** Measured same-session on `node_kill_round.sh` (3-node RF=3, `kill -9` mid-bench), final binary vs a control differing only in the flag: **62/400 and 69/400 failed batches with a 12 s recovery (2.25-2.31 M pts/s) ON, against 49/400 twice with an 8 s recovery (3.25-3.30 M) OFF.** The remaining gap is one extra campaign cycle: the hibernation wake (D-9, `277e110`) fires on the FIRST vote the lease drops, so that vote is still lost and the candidate must come round again at its own election timeout. Two candidate fixes, neither attempted: (a) wake the groups behind an unreachable peer on **both** sides — today only the node whose own writes fail does it, so the peer that must VOTE is still hibernating when the first vote arrives; (b) make the lease real-time rather than tick-counted (e.g. advance the election clock by the number of PASSES skipped in one call, which would also delete the hibernation-stretched failover that `wakeFollowersOf` exists to paper over). (a) is smaller; (b) is the actual defect. Contract is unaffected either way — no 500s, no acknowledged loss, and the gate asserts both. | `raft_group_registry.cpp` (`deliver`, `tickAll`), `replicated_write_router.cpp` (`wakeGroupsBehind`); gate `test/cluster_gates/node_kill_round.sh` | consensus owner |
| D-30 | **ADR 0005's mechanism (c) — gated activation — is NOT implemented, so CheckQuorum is a per-binary decision.** The ADR's recommendation was (b) AND (c): the new type byte makes the WIRE safe (an old voter drops a transfer vote rather than misparsing it), but only a cluster-wide committed format version makes the BEHAVIOUR safe, by keeping CheckQuorum off until every voter can read that byte. Without (c), a rolling upgrade has a window in which a new node runs the guard while an old peer drops its transfer votes: transfers in that window revert to the pre-bypass speed (abandon after one election timeout, then a normal election). Degraded, never unsafe — and the same shape as D-7, which also waits on group-0's `activeFormatVersion` being wired to the codec, so they should land together. | `lib/cluster/control/group0_state.hpp` (`activeFormatVersion`), `features::FeatureGate::canActivate`; `cluster_data_plane.cpp` (the construction site) | consensus owner |
