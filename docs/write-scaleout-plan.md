...
|---|---|---|---|
| ~~D-1~~ | **CLOSED** (`9b0ae8a`). Exact equality (`matchIndexOf(target) == log().lastIndex()`) is replaced by two gates, both required: a **lag bound** (within 64 entries, ~one AppendEntries round trip of in-flight work) and a **liveness bound** (`RaftNode::ticksSinceAck` within 3 heartbeat intervals). The liveness half is new and is the part a lag bound cannot supply: on a write-IDLE group a dead peer sits at lag **zero** indefinitely — it was caught up when it died and lastIndex never moves again — so it passes any delta test *including the old exact equality*. That hole is pre-existing; this closes it. The clock is one monotonic counter plus a per-peer stamp written where matchIndex already is (O(1) per tick, which matters at 4096 groups/shard every 20 ms), cleared on every role change and deliberately unseeded on `becomeLeader` so a fresh leader reads every peer as dead until it answers. Residual, bounded: a peer dying between its last ack and the current pass can still be targeted, costing that group its proposals for ONE election timeout before `RaftNode::tick` abandons the transfer (§3.10) — one window on the pass that races the death, not one per pass forever. **MEASUREMENT, and it is a null result:** `rolling_rebalance_gate.sh` reports **2216 transfers / 102 calls / 600 of 600 requests OK / converged to [1364 1368 1364]** — and a binary built with D-1 reverted produces *exactly the same numbers*. The gate is INSENSITIVE to this change, because its load (1000 hosts over 4096 groups) leaves almost every group momentarily idle at any given pass, so exact equality was rarely the binding constraint there. The discriminating workload is a SKEWED one where a few groups take all the writes, and no gate covers it — see D-18. The semantics are pinned at unit level instead (`RaftPeerLivenessTest`, 7 cases). | `lib/cluster/integration/shard_raft_plane.hpp` (`rebalance`); `raft_node.{hpp,cpp}` (`ticksSinceAck`) | movement/M5 |
