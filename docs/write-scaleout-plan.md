...
#### 5.3 / 5b. Shard-level fsync coalescing — the premise did not hold; the real 5b is now BUILT, DEFAULT OFF (`b7510f6`, see D-10)
...
(c) retention stays unwired in both layouts — re-scoped into D-34. Evidence is a
...
tmpfs. Remaining before default-on: a real-disk measurement, and D-34. See the D-10
register row.
...
**State as of 2026-07-27 (`b7510f6`, tip of `cluster-design`):** closed so far — D-1,
D-2, D-3, D-4 (two of three halves; third re-filed as D-19), D-5, D-6, D-7, D-11, D-12,
D-13, D-15, D-16; D-14 materially improved. D-10 is BUILT but ships default-off and
unmeasured on real disk (see its row — three named gates before default-on, one of which
is D-34). **The standout open correctness item is D-36** (acked writes not readable after
a restart that follows a heavy write campaign; loss vs apply-stall undetermined) — it
predates the snapshot work and nothing in this campaign has touched it. Suites at last
green: unit 4205, socket 37.
...
| ~~D-17~~ | **CLOSED** (`1e9ffad`). Dirty index caches accumulated with no bound between flushes: the four application caches (postings bitmaps, day bitmaps, HLL sketches, measurement blooms) are mutated WITHOUT writing a KV entry, and `maybeFlushMemTable()` returns immediately unless the MEMTABLE crossed its threshold -- so the trigger measured the one quantity that was not accumulating. A day rollover adds real membership bits to brand-new day bitmaps while creating no series; steady state on a fixed fleet re-touches known series and grows the memtable barely or not at all. Neither bytes nor time bounded the result: the trims deliberately SKIP dirty entries (evicting one would discard an unpersisted add), so a dirty entry is trim-exempt and the byte budgets cannot reclaim it; and everything outstanding is lost on a non-clean exit, with day-bitmap membership having no repair path on open the way postings do (the watermark repair rebuilds postings from metadata; a day bitmap's insert timestamps are gone). D-3 closed only the close()-time half. **Two triggers, one flush body:** the block both flush paths duplicated is extracted as `flushDirtyCaches()` (documented precondition: caller holds `flushMutex_`, because it clears every `dirty` flag and the batch is then the only copy of those adds) and the new triggers call THAT rather than adding a second way for cache state to reach disk. Its apply-before-append ordering is unchanged and now pinned, since inverting it is exactly how D-2/D-3 lost data. **(a) SIZE**, checked O(1) on the insert path via `maybeFlushDirtyCaches()` -- after every day-recording loop (`recordDaySpan`, `recordInsertDays`, `indexInsert`) and on the under-threshold branch of `maybeFlushMemTable()`, i.e. exactly the paths that write no KV entry and would otherwise never ask. Deliberately not a coroutine (the answer is almost always "nothing to do"; a coroutine would allocate a frame per call to reach a bare `co_return`). Thresholds: **10,000 dirty keys** -- one tenth of `MAX_BITMAP_CACHE_ENTRIES`, so the entry-count trims can still reclaim 90% of the cache -- or **10% of `indexCacheBudgetBytes()`**, the slice `maxHllCacheBytes()` already gets (~51 MB at the 512 MB ceiling, ~1.6 MB at the 16 MB floor), charged as each key ENTERS its dirty set. **(b) AGE**, a **1 s** periodic flush, because the size bound provably cannot fire in the steady state that motivated the item (one day bitmap re-touched forever is ONE key). 1 s is 10x `kWalSyncInterval`: the WAL sync flushes a buffer and fdatasyncs, this serializes changed bitmaps into a batch, so it is deliberately an order of magnitude coarser. The timer consults `hasDirtyCacheState()`, which is STRICTER than `flushDirtyCaches()`'s own entry guard, so an idle shard with populated-but-clean caches does not rewrite the local-ID counter and watermark (and dirty the memtable) once a second forever. Armed at the END of `open()` -- the migration and crash-window repair above it dirty these very caches -- cancelled AND gate-drained in `close()` before the close-time flush (it takes `flushMutex_` and appends to the WAL `close()` is shutting down), and cancelled again in the destructor, the shape `walSyncTimer_` already had. **(c) An add that changes nothing no longer dirties.** The cache-HIT paths of `addToPostingsBitmapForInsert`/`addToDayBitmapForInsert` marked their entry dirty unconditionally, so re-inserting an already-present local ID pinned it against the trims and left the next flush re-serializing a bitmap byte-identical to the one on disk. They now use `addChecked` and dirty only on a real change -- which is what makes the age bound affordable, and is safe precisely there because neither path suspends. The COLD paths still re-mark unconditionally (a flush during their load can clear the flag while ids they cannot see were added to the pinned entry -- the 4d/D-3 hazard). **Residuals, both bounded and deliberate:** the byte estimate charges only the clean->dirty transition and does not follow a bitmap growing IN PLACE while already dirty (a branch on the index's hottest path for a figure that would still be an estimate; the age bound covers it, since a bitmap growing in place is by definition being changed); and the size check is per-CALL, so one call's day loop can overshoot before the check runs -- bounded by 366 keys for `recordDaySpan` (the `kMaxDaySpan` clamp) and by the batch's distinct-day count for the other two. The crash window is now ~1.1 s (one flush interval plus the 100 ms WAL sync) rather than unbounded, not zero: `flushDirtyCaches()` appends to the index WAL, it does not fsync. **Gate:** `dirty_cache_flush_trigger_test.cpp`, the INVERSE of `day_bitmap_concurrent_insert_test.cpp`'s tiny-buffer trick -- a 64 MB write buffer so the memtable trigger cannot fire at all. All four tests fail with the change reverted: 20,003 dirty keys against a bound of 10,000 (`SizeTriggerBoundsDirtyCachesWithoutMemtablePressure`, which also reopens and reads back the FIRST day it walked, the one a dirty-cleared-before-durable regression would lose while still looking healthy on the last), dirty state surviving 10 s idle (`AgeTriggerFlushesDirtyCachesWithNoFurtherWrites`), 500 identical re-inserts dirtying the caches (`RepeatedIdenticalInsertsDirtyNothing`), and the structural pin on apply-before-append plus the timer lifecycle (`DirtyCacheFlushSourceInspection.FlushOrderingAndTimerLifecycleAreIntact`). Unit **4209/4209**, cluster socket **37/37**. | `native_index.cpp`/`.hpp` (`flushDirtyCaches`, `flushDirtyCachesUnderMutex`, `maybeFlushDirtyCaches`, `hasDirtyCacheState`, `noteDirtyBitmapKey`, `dirtyCacheTimer_`, `kMaxDirtyCacheKeys`/`kDirtyCacheBudgetPercent`/`kDirtyCacheFlushInterval`); gate `test/unit/index/dirty_cache_flush_trigger_test.cpp` | index owner. Pre-existing |
...
| D-34 | **Journal SEGMENT reclamation is unwired, so compaction bounds replay but not disk.** D-6 compacts the in-memory Raft log and persists a Snapshot record, so `recoverRaftState` drops entries at or below the boundary on restart -- replay is bounded. But the sealed journal segments holding those entries are still on disk and nothing deletes them; a long-lived cluster's `cluster_raft/` grows without limit regardless of how often it snapshots. `JournalGc` exists as a brick (`lib/storage/journal_gc.hpp`, with `JournalRetention`) and was never given a caller. Wants the snapshot boundary published as the reclaim floor, plus the copy-forward rule the brick already documents. **Re-scoped by D-10 (`b7510f6`):** reclamation is unwired in BOTH journal layouts, so the shared journal regresses nothing — and the bricks were WRITTEN for the shared layout (their headers describe per-core multiplexing and the laggard-pins-segment problem copy-forward solves). Under the shared journal a segment is reclaimable only once EVERY group's boundary has passed it — `planSegment`'s rule verbatim. Wiring segment DELETION is a second durability change and was deliberately kept out of the D-10 commit; it is also a gate on flipping the shared journal default-on. | `lib/storage/journal_gc.{hpp,cpp}`; producer at `replicated_vshard_host.cpp` (`snapshotVShard`) | snapshot owner |
