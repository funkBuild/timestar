...
**State as of 2026-07-27 (`1375bd0`, tip of `cluster-design`):** closed so far — D-1,
...
it. Suites at last green: unit 4231, socket 37.
...
| D-39 | **Shared-journal segment reclamation is rate-limited by the SNAPSHOT sweep, and head-of-line blocked by the slowest group.** Filed while closing D-34; it only bites with `TIMESTAR_CLUSTER_SHARED_JOURNAL=1`. A segment there holds every group on the reactor, so it is reclaimable only once EVERY group's floor has passed every record in it -- or the stragglers are copied forward, which D-34 caps at `maxCopyForwardRecords` = 512 so a mostly-live segment is pinned instead. GC then STOPS at that pin (D-34, for suffix-contiguity), so ONE lagging group holds up the whole shard's reclamation, not just its own segment. A floor moves when its group compacts, and D-6's sweep takes at most **one snapshot per shard per 5 s** with a 60 s per-group minimum over ~1365 groups per shard -- **~1.9 hours per full cycle**. To be precise about what that costs, because the first version of this row overstated it: the floor-0 population is a WARM-UP effect, not a steady state (every group crosses its threshold eventually and thereafter re-snapshots on the 60 s minimum), so the steady-state complaint is the LATENCY of the cycle and the head-of-line block, not that most groups are permanently at zero. The per-VShard default shares the same sweep and therefore the same latency; what it does NOT have is the conjunction -- its segments hold one group, so a slow group delays only its own directory. **Wants** either a higher `kMaxConcurrentSnapshots` / shorter sweep when the shared journal is on (the cost is reading and encoding N VShards' TSM files on one reactor, which is what the rate limit exists to prevent), or a cheap floor-advance that does not require producing a payload at all -- the flush watermark is already known, and a floor could be published from it without encoding a snapshot. **Evidence to gather first:** `journalSegmentsPinned()` vs `journalSegmentsDeleted()` on a shared-journal gate run; nothing measures it today. | `replicated_vshard_host.hpp` (`kSnapshotSweepInterval`, `kMaxConcurrentSnapshots`, `kMinSnapshotInterval`); `journal_gc.hpp` (`maxCopyForwardRecords`, the stop-at-pin rule) | snapshot owner |
| D-40 | **VShard teardown must retire its reclaim floor, in BOTH directions, and nothing calls the hook.** Filed while closing D-34. **Correcting the first version of this row, which asserted in the present tense that "the per-VShard layout deletes the VShard's directory outright on move": IT DOES NOT, and no such code exists anywhere in the tree** -- there is no `removeVShard`, nothing unlinks `vshard_N/`, and `ReplicatedVShardHost` has no teardown for a single group at all. VShard movement teardown is unwired (M5), so what follows is what that wiring MUST do, not a description of today. **Direction 1, the leak (shared layout):** the reclaim floor is published per VShard from the group's own `JournalRaftPersistence`, so a group that stops being hosted has nobody to advance it; `retention_` keeps its last value (the map is monotonic and nothing removes entries) and every shared segment holding a record above that value is pinned for the life of the process -- and, since GC stops at the first pin, so is everything after it. Teardown must publish "everything released" for the departing VShard. **Direction 2, the DELETION, which is the dangerous one and applies to BOTH layouts:** `setReleased` is monotonic and ignores a regression, which is right while one sequence climbs and lethal the moment the sequence RESTARTS. A VShard removed and later re-added gets a fresh journal whose `vshard_seq` begins at 1 (a new directory in the per-VShard layout; new records in a live file in the shared one), and a stale watermark of, say, 500 marks the ENTIRE new journal released -- the first GC pass deletes the new group's sealed segments, snapshot boundary and only HardState record included. `JournalRetention::clearReleased` exists for exactly this and is deliberately UNCALLED, with the reasoning at its declaration, so the wiring is one line at the teardown site rather than a rediscovery. **Why it must hang off the movement decision and not off `vshards_.count() == 0`:** a group that is merely momentarily absent -- an `addVShard` that has not run yet, a restart mid-move -- would otherwise be declared fully released and have its live log deleted. | `replicated_vshard_host.cpp` (`publishReclaimFloors`, `retention_`), `journal_retention.hpp` (`clearReleased`); movement/M5 VShard teardown | movement/M5 + snapshot owner |
