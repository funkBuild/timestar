...
**State as of 2026-07-31 (last code commit of the D-28/D-20/D-24 chain: `8d0ffcb`):** closed so far — D-1,
D-2, D-3, D-4 (two of three halves; third re-filed as D-19), D-5, D-6, D-7, D-11, D-12,
D-13, D-15, D-16, D-17, D-20, D-24, D-28, D-33, D-34, D-35; D-14 materially improved.
...
rather than evidence in hand — read it before treating the timing as settled. D-10 is BUILT but
...
it. Suites at last green: unit 4242, socket 37.
...
| ~~D-20~~ | **CLOSED** (`5d8d2d8`). The window is now `RaftOptions::transferTimeout` — 0 => `2 * heartbeatTimeout`, clamped to at least one tick and never past `electionTimeoutMin` — and it is wired explicitly in `ClusterDataPlane::start` at **50 ticks (1 s)** with the same fail-closed startup assertion the snapshot chunk timeout has, so the ordering `transferTimeout 50 < kDeadline 1.5 s << electionTimeoutMin 125` cannot drift silently. Clamped against the MINIMUM rather than the current randomized draw, so "the transfer window never outlasts an election" holds for every draw this node will make. **Why shortening it is safe, and it is a claim about Raft rather than about this code:** abandoning is a purely LOCAL decision by the OLD leader to resume proposing in a term it already leads — it retracts nothing and grants nothing. A transferee holding a TimeoutNow campaigns at term+1, which no leader holds, so two leaders in one term remains impossible; if it wins we step down on the higher term exactly as for any lost election, and the proposals accepted in between either committed on a quorum (in which case §5.4.1 guarantees the winner has them — a candidate missing a committed entry cannot collect a majority) or did not (truncated, and never acked, since `proposeAndAwaitApplied` resolves on APPLY and the router derives uncommitted from the committed set). The cost of abandoning a transfer that WOULD have completed is only that it did not happen: no TimeoutNow fires on a later catch-up ack and the next balancer pass tries again. **THE FAULT-GATE MEASUREMENT THIS ROW ASKED FOR WAS NOT RUN** — this environment has no live gates — so the D-1 precedent (a consensus-timing change whose 3-heartbeat variant the gate REJECTED) is answered here by deterministic tick-sequence coverage instead, six tests in `raft_transfer_abort_test.cpp`: refusal lasts the whole window and resumes at its end; the bound's derivation, override, clamp and never-zero floor; a target that acks ONE FULL HEARTBEAT ROUND later still gets TimeoutNow (an ABSOLUTE bound — a loop sized from the window under test shrinks with it and passes at any window at all, which is exactly what the first draft did); abandon-then-transferee-wins (we step down, the committed write survives, and we REFUSE the vote per §5.4.1); abandon with no TimeoutNow ever delivered (term unmoved, group resumes committing on its live 2-of-3); and a proposal inside the window still presenting the LeaderRefused shape `classifyRefusal` reads, so the write path's retry policy is unchanged. Negative controls run: window back to `electionTimeout_` → 5 of 9 fail; window forced to 1 tick → 3 fail; clamp removed → the bound test fails 10000 vs 20; and the §5.4.1 assertion checked for vacuity by giving the candidate the committed entry. **Residual: run `fault_injection_gate.sh` and `node_kill_round.sh` against this binary on a box that has them** — D-21's variance note applies, so K runs and an aggregate, not one draw. A second-order effect worth watching there: the balancer may now re-arm a window for the same target on a later pass, so the refusal is ~1 s per pass rather than one long block (D-1's liveness filter stops the re-pick once the ack clock decays). | `lib/cluster/raft/raft_node.{hpp,cpp}` (`transferTimeout`, the §3.10 abandon arm); `cluster_data_plane.cpp` (the ordering assertion) | consensus owner |
...
| ~~D-24~~ | **CLOSED** (`8d0ffcb`). `RaftNode::transferLeadership` returns whether `leadTransferee_` went from unset to set — false from EVERY early return (not leader, self, non-voter, and the F2 re-arm guard this row is about) — `RaftGroup::transferLeadership` propagates it instead of discarding it, and `rebalance` charges both `done` and the target's `deficit` on that flag. The deficit matters as much as the counter: spending it on an unarmed transfer makes the rest of the pass skip a peer that still needs groups. The re-arm door is reachable on the ORDINARY schedule, not just under operator storms — the balancer re-picks the same peer while its deficit stands, on a period comparable to the abandon window — so `transfers_initiated` counted one transfer per PASS where one was armed per WINDOW, and `deposed_primary_gate.sh`'s anti-vacuity FLOOR was correspondingly weaker than it reads. Pinned by `RaftTransferAbortTest.{TransferLeadershipReportsWhetherItStartedOne, RepeatedPassesCountOneTransferPerWindowNotOnePerPass}` — the second runs the BALANCER'S OWN call pattern and compares the reported count against ground truth sampled independently (`transferInFlight()` transitions), so it cannot pass by trusting the value under test, plus two anti-vacuity bounds — and by `RaftGroupTest.TransferLeadershipReportsThroughTheDriver`, because a driver that swallowed the answer would leave the counter inflated whatever the core reports. Negative controls run: re-arm guard reporting `true` (the pre-fix inflation) fails 3 tests with "counted 12 transfers where 4 were armed"; the driver forced to `co_return true` fails the seam test. **Residual: the `rebalance` LOOP itself is still not unit-testable** (it needs a whole `ReplicatedDataPlane`), so the counting RULE is pinned at the two seams the loop consumes rather than in the loop — that is D-22's extraction, and D-22 should adopt this assertion when it lands. | `raft_node.{hpp,cpp}`, `raft_group.{hpp,cpp}` (the report); `shard_raft_plane.hpp` (`rebalance`); gate `deposed_primary_gate.sh` | consensus owner |
